...
We actively support the following versions of Reasongraph with security updates:
...
Create a [GitHub Security Advisory](https://github.com/savsuth/Reasongraph/security/advisories/new) or contact us via the security email listed in `SUPPORT.md`.
...
Reasongraph's build and release pipeline is explicitly hardened against
...
- **Safety** — known CVEs in Reasongraph's own installed dependencies, including optional LLM-provider extras such as LiteLLM; CI fails on any match. Runs in `security-scan.yml` on every push/PR to `main` and twice weekly.
...
Teams standing up their own instance of Reasongraph, or forking it for an
...
2. Report to us if it affects Reasongraph specifically
...
1. **Keep Reasongraph Updated**: Always use the latest stable version
...
We appreciate responsible disclosure. Security researchers who help us improve the security of Reasongraph will be:
...
- **GitHub Security Advisories**: [Report vulnerability](https://github.com/savsuth/Reasongraph/security/advisories/new)
...
**Thank you for helping keep Reasongraph and its users safe!**
