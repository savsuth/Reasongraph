...
DISABLE_JUPYTER_PROGRESS = os.getenv("REASONGRAPH_DISABLE_JUPYTER_PROGRESS", "").strip().lower() in (
    "1",
    "true",
    "yes",
    "on",
...
def _progress_disabled_from_env() -> bool:
    ...
    return os.getenv("REASONGRAPH_DISABLE_PROGRESS", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
class ConsoleProgressDisplay(ProgressDisplay):
    ...
    def _get_action_message(self, module: Optional[str], message: str) -> str:
        ...
        base_msg = f"Reasongraph {action}"

        if not message:
            return base_msg
            
        # If message already contains Reasongraph format, use it
        if "Reasongraph:" in message:
            return message.split("Reasongraph:")[-1].strip()
        ...
        with self.lock:
            ...
            if pipeline_items:
                ...
                if not hasattr(self, '_pipeline_header_shown'):
                    if self.use_emoji:
                        self._safe_write("🧠 Reasongraph - 📊 Current Progress\n")
                    else:
                        self._safe_write("Reasongraph - Current Progress\n")
    ...
    def show_summary(self, items: List[ProgressItem]) -> None:
        ...
        with self.lock:
            ...
            # Summary header
            if self.use_emoji:
                self._safe_write("🧠 Reasongraph - 📊 Progress Summary\n")
            else:
                self._safe_write("Reasongraph - Progress Summary\n")
            self._safe_write("=" * 80 + "\n")
...
class JupyterProgressDisplay(ProgressDisplay):
    ...
    def _get_action_message(self, module: Optional[str], message: str) -> str:
        """Get action message based on module."""
        # Extract action from message if it contains "Reasongraph:"
        if message and "Reasongraph:" in message:
            # Use the message as-is if it already has Reasongraph format
            return (
                message.split("Reasongraph:")[-1].strip()
                if "Reasongraph:" in message
                else message
            )
    ...
    def _build_html(self, items: List[ProgressItem]) -> str:
        ...
        # Current status
        html_parts.append("<h4>🧠 Reasongraph - 📊 Current Progress</h4>")
    ...
    def show_summary(self, items: List[ProgressItem]) -> None:
        ...
        html_parts = ["<div style='font-family: monospace;'>"]
        html_parts.append("<h3>🧠 Reasongraph - 📊 Progress Summary</h3>")
...
class FileProgressDisplay(ProgressDisplay):
    ...
    def show_summary(self, items: List[ProgressItem]) -> None:
        ...
        self.logger.info("=" * 80)
        self.logger.info("REASONGRAPH - PROGRESS SUMMARY")
        self.logger.info("=" * 80)
...
class ModuleDetector:
    ...
    def detect_from_frame(frame) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            # Extract module name (e.g., 'reasongraph.ingest.file_ingestor' -> 'ingest')
            if "reasongraph." in module_name:
