...
capabilities for the Reasongraph framework, supporting multiple RDF serialization
...
class NamespaceManager:
    ...
    def __init__(self, **config):
        ...
        # Standard RDF namespaces. "reasongraph" reuses ProvenanceManager's
...
class RDFSerializer:
    ...
    # reasongraph:openEndedInterval "true"^^xsd:boolean on the time:Interval
    ...
    def serialize_to_turtle(self, rdf_data: Dict[str, Any], **options) -> str:
        ...
        base_namespaces = {
            "rdf": "http://www.w3.org/1999/02/22-rdf-syntax-ns#",
            "xsd": "http://www.w3.org/2001/XMLSchema#",
            "reasongraph": "https://reasongraph.dev/ns#",
        }
        if include_temporal:
        ...
        for entity in entities:
            ...
            if not entity_id:
                ...
                entity_id = f"reasongraph:entity_{hash(entity_text)}"

            entity_type = entity.get("type", "reasongraph:Entity")
            text = entity.get("text") or entity.get("label", "")
            confidence = entity.get("confidence", 1.0)

            lines.append(f"<{entity_id}> a <{entity_type}> ;")
            lines.append(f'    reasongraph:text "{text}" ;')
            lines.append(f"    reasongraph:confidence {confidence} .")
            lines.append("")
        ...
        for idx, rel in enumerate(relationships):
            ...
            rel_type = rel.get("type", "reasongraph:related_to")
    ...
    def _owl_time_triples_for_rel(
        ...
        For TemporalBound.OPEN valid_until values we use:
            reasongraph:openEndedInterval "true"^^xsd:boolean
        ...
        rel_base_id = (
            rel.get("id")
            or f"reasongraph:rel_{idx}_{hash(str(rel.get('source_id', '')) + str(rel.get('target_id', '')))}"
        )
    ...
    def serialize_to_rdfxml(self, rdf_data: Dict[str, Any], **options) -> str:
        ...
        lines.append('         xmlns:reasongraph="https://reasongraph.dev/ns#">')
        lines.append("")
        ...
        for entity in entities:
            ...
            lines.append(f"    <reasongraph:text>{text}</reasongraph:text>")
            lines.append(
                f"    <reasongraph:confidence>{confidence}</reasongraph:confidence>"
            )
    ...
    def serialize_to_jsonld(self, rdf_data: Dict[str, Any], **options) -> str:
        ...
        jsonld = {
            "@context": {
                "@vocab": "https://reasongraph.dev/vocab/",
                "reasongraph": "https://reasongraph.dev/ns#",
                "rdf": "http://www.w3.org/1999/02/22-rdf-syntax-ns#",
                "rdfs": "http://www.w3.org/2000/01/rdf-schema#",
            },
        ...
        for entity in entities:
            ...
            if not entity_id:
                entity_text = entity.get("text", "")
                entity_id = f"reasongraph:entity/{entity_text}"

            jsonld["@graph"].append(
                {
                    "@id": entity_id,
                    "@type": entity.get("type", "reasongraph:Entity"),
                    "reasongraph:text": entity.get("text") or entity.get("label", ""),
                    "reasongraph:confidence": entity.get("confidence", 1.0),
                }
            )
        ...
        for rel in relationships:
            ...
            if not rel_id:
                ...
                rel_id = f"reasongraph:rel/{source_id}_{target_id}"

            jsonld["@graph"].append(
                {
                    "@id": rel_id,
                    "@type": "reasongraph:Relationship",
                    "reasongraph:source": {
                    ...
                    "reasongraph:type": rel.get("type", "related_to"),
                }
            )
    ...
    def serialize_to_ntriples(self, rdf_data: Dict[str, Any], **options) -> str:
        ...
        def expand_uri(uri: str) -> str:
            ...
            if uri.startswith("http"):
                return f"<{uri}>"
            if uri.startswith("reasongraph:"):
                return f"<https://reasongraph.dev/ns#{uri.split(':', 1)[1]}>"
            ...
            if ":" in uri:
                return f"<{uri}>"
            return f"<https://reasongraph.dev/ns#{uri}>"
        ...
        for entity in entities:
            ...
            if text:
                ...
                lines.append(
                    f'{subject} {expand_uri("reasongraph:text")} "{safe_text}" .'
                )
            ...
            if confidence is not None:
                lines.append(
                    f'{subject} {expand_uri("reasongraph:confidence")} "{confidence}"^^<http://www.w3.org/2001/XMLSchema#float> .'
                )
