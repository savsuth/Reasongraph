...
Reasongraph framework, enabling structured data export for knowledge graphs and
...
class JSONExporter:
    ...
    def export_entities(
        ...
        json_data = {
            "@context": {
                ...
                "entities": {"@id": "reasongraph:entities", "@container": "@list"},
            },
            "entities": entities,
            "metadata": {
    ...
    def _convert_to_jsonld(
        ...
        **options,
        ...
        if include_metadata:
            jsonld["@id"] = f"https://reasongraph.dev/data/{datetime.now().isoformat()}"
            if include_provenance:
                jsonld["reasongraph:exportedAt"] = datetime.now().isoformat()
                jsonld["reasongraph:format"] = "json-ld"
    ...
    def _convert_kg_to_jsonld(self, kg: Dict[str, Any], **options) -> Dict[str, Any]:
        ...
        jsonld = {
            ...
            "@id": f"https://reasongraph.dev/graph/{datetime.now().isoformat()}",
            "@type": "reasongraph:KnowledgeGraph",
        }

        # Convert entities to JSON-LD format
        entities = kg.get("entities", [])
        if entities:
            jsonld["reasongraph:entities"] = [self._entity_to_jsonld(e) for e in entities]
            self.logger.debug(f"Converted {len(entities)} entity(ies) to JSON-LD")

        # Convert relationships to JSON-LD format
        relationships = kg.get("relationships", [])
        if relationships:
            jsonld["reasongraph:relationships"] = [
        ...
        jsonld["reasongraph:exportedAt"] = datetime.now().isoformat()
        if "metadata" in kg:
            jsonld["reasongraph:metadata"] = kg["metadata"]
    ...
    def _entity_to_jsonld(self, entity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        jsonld = {
            "@id": entity_id,
            "@type": entity.get("type") or "reasongraph:Entity",
        ...
        if "metadata" in entity:
            jsonld["reasongraph:metadata"] = entity["metadata"]
    ...
    def _relationship_to_jsonld(self, rel: Dict[str, Any]) -> Dict[str, Any]:
        ...
        jsonld = {
            ...
            "reasongraph:source": {"@id": rel.get("source_id") or rel.get("source")},
            "reasongraph:target": {"@id": rel.get("target_id") or rel.get("target")},
            "reasongraph:confidence": rel.get("confidence", 1.0),
        }

        # Add metadata if present
        if "metadata" in rel:
            jsonld["reasongraph:metadata"] = rel["metadata"]
