...
2. [Reasongraph Class](#reasongraph-class)
...
```python
from reasongraph.core import build
...
```python
from reasongraph.core import Reasongraph, Config
...
framework = Reasongraph(config=config_dict)
...
- `ReasongraphError`: If initialization fails
...
```python
from reasongraph.core import Config
...
**Raises:**
- `ReasongraphError`: If startup fails
...
**Raises:**
- `ReasongraphError`: If shutdown fails and graceful=False
...
```python
from reasongraph.core.methods import run_pipeline
...
Initialize Reasongraph framework.

```python
from reasongraph.core.methods import initialize_framework
...
**Returns:**
- Initialized `Reasongraph` framework instance
...
```python
from reasongraph.core.methods import get_status
...
**Parameters:**
- `framework`: Optional Reasongraph framework instance (creates new if None)
...
```python
from reasongraph.core.methods import get_orchestration_method
...
```python
from reasongraph.core.methods import list_available_methods
...
Configuration can be loaded from environment variables with `REASONGRAPH_` prefix:

```bash
export REASONGRAPH_PROCESSING_BATCH_SIZE=64
export REASONGRAPH_LLM_PROVIDER_MODEL=gpt-4
export REASONGRAPH_QUALITY_MIN_CONFIDENCE=0.8
```
...
from reasongraph.core import Reasongraph
framework = Reasongraph(config=merged)
```

## Best Practices

1. **Always Initialize**: Always call `initialize()` after creating a `Reasongraph` instance before using it.
