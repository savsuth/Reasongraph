...
_MAX_SHACL_TURTLE_BYTES: int = int(
    os.environ.get("REASONGRAPH_MAX_SHACL_TURTLE_BYTES", "262144")
)  # 256 KB
_MAX_SHACL_TRIPLES: int = int(
    os.environ.get("REASONGRAPH_MAX_SHACL_TRIPLES", "1000")
)  # 1,000 triples
_MAX_SHACL_TIMEOUT_SECONDS: float = float(
    os.environ.get("REASONGRAPH_MAX_SHACL_TIMEOUT", "15.0")
)
_MAX_SHACL_CONCURRENCY: int = int(
    os.environ.get("REASONGRAPH_MAX_SHACL_CONCURRENCY", "4")
)
...
def _parse_rdf_sync(content: bytes, fmt: str) -> tuple:
    ...
    if "uri" not in metadata:
        metadata["uri"] = f"urn:reasongraph:onto:{uuid.uuid4().hex[:8]}"
