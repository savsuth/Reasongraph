"""
Reasongraph Knowledge Explorer : CLI Entry Point

Provides the ``reasongraph-explorer`` command that loads a graph from a
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        prog="reasongraph-explorer",
        description="Reasongraph Knowledge Explorer — interactive dashboard for KG exploration",
    )
    parser.add_argument(
        "--graph", "-g",
        required=True,
    ...
    try:
        ...
        _err.print(
            ...
            "  [dim]pip install reasongraph[explorer][/dim]"
        )
        sys.exit(1)
    ...
    if args.host not in _LOOPBACK_HOSTS:
        import os as _os
        if _os.environ.get("REASONGRAPH_ALLOW_ANONYMOUS", "").strip().lower() == "true":
            _err.print(
                f"[bold yellow]Warning:[/bold yellow] Binding to "
                f"[cyan]{args.host}[/cyan] with REASONGRAPH_ALLOW_ANONYMOUS=true "
                "exposes the Explorer to the network with no authentication — "
                "all graph data will be readable and writable by any host that "
                "can reach this port."
            )
        elif not _os.environ.get("REASONGRAPH_API_KEY"):
            _err.print(
                f"[bold yellow]Warning:[/bold yellow] Binding to "
                f"[cyan]{args.host}[/cyan] but REASONGRAPH_API_KEY is not set — "
    ...
    _out.print(
        Panel(
            ...
            title=f"[bold]Reasongraph Explorer[/bold] · [dim]{url}[/dim]",
            border_style="cyan",
            expand=False,
        )
    )
