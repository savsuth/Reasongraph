"""
Reasongraph Explorer : FastAPI Dependencies
...
def get_expected_api_key() -> Optional[str]:
    ...
    """
    return os.environ.get("REASONGRAPH_API_KEY") or None


def anonymous_access_allowed() -> bool:
    return os.environ.get("REASONGRAPH_ALLOW_ANONYMOUS", "").strip().lower() == "true"
...
def require_auth(api_key: Optional[str] = Security(_api_key_header)) -> None:
    ...
    REASONGRAPH_API_KEY is unset, requests are refused with 503 rather than
    silently served unauthenticated — REASONGRAPH_ALLOW_ANONYMOUS=true opts
    ...
    if not expected:
        raise HTTPException(
            ...
            detail=(
                ...
                "REASONGRAPH_API_KEY environment variable, or explicitly opt "
                "into unauthenticated access (development only) with "
                "REASONGRAPH_ALLOW_ANONYMOUS=true."
            ),
        )
