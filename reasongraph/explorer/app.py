"""
Reasongraph Explorer FastAPI application factory.
"""
...
def create_app(
    ...
    async def lifespan(app: FastAPI):
        ...
        if anonymous_access_allowed():
            _lifespan_logger.warning(
                "Explorer is running with REASONGRAPH_ALLOW_ANONYMOUS=true — "
                "all API routes are unauthenticated. Do not expose this "
                "process beyond localhost."
            )
        elif get_expected_api_key():
            _lifespan_logger.info("Explorer API authentication: enabled (REASONGRAPH_API_KEY set).")
        else:
            _lifespan_logger.warning(
                "Explorer API authentication: NOT CONFIGURED. All protected "
                "routes will return 503 until REASONGRAPH_API_KEY is set."
            )
    ...
    app = FastAPI(
        title="Reasongraph Knowledge Explorer",
        description="Interactive dashboard API for exploring Reasongraph knowledge graphs.",
        version=__version__,
        lifespan=lifespan,
    )
    ...
    async def websocket_endpoint(websocket: WebSocket):
        ...
        # CORS support only wraps HTTP), so under REASONGRAPH_ALLOW_ANONYMOUS
        ...
        # (browser clients), same REASONGRAPH_API_KEY the REST routes check.
    ...
    async def root():
        ...
        return HTMLResponse(
            ...
            '<title>Reasongraph Knowledge Explorer</title>'
            ...
            "<code>pip install reasongraph[explorer]</code>, or build from source with "
