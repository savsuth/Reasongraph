"""
Reasongraph Server Entry Point

This module provides the REST API server for the Reasongraph framework
...
async def lifespan(app: FastAPI):
    ...
    logging.info("Starting up Reasongraph API...")

    if EXPLORER_AVAILABLE:
        if anonymous_access_allowed():
            logging.warning(
                "REASONGRAPH_ALLOW_ANONYMOUS=true — all Explorer API routes are "
                "unauthenticated. Do not expose this process beyond localhost."
            )
        elif get_expected_api_key():
            logging.info("Explorer API authentication: enabled (REASONGRAPH_API_KEY set).")
        else:
            logging.warning(
                "Explorer API authentication: NOT CONFIGURED. Protected routes "
                "will return 503 until REASONGRAPH_API_KEY is set."
            )
        try:
    ...
    logging.info("Shutting down Reasongraph API...")
...
app = FastAPI(
    ...
    description="REST API for the Reasongraph Framework",
    version=__version__,
    lifespan=lifespan,
)
...
_cors_origins_env = os.environ.get(
    "REASONGRAPH_CORS_ORIGINS", "http://localhost:5173,http://127.0.0.1:5173"
)
...
if EXPLORER_AVAILABLE:
    ...
    logging.warning(
        ...
        "install the required dependencies: pip install 'reasongraph[explorer]'."
    )
...
async def serve_spa(full_path: str):
    ...
    if full_path in ("", "/"):
        ...
        return JSONResponse({
            "name": "Reasongraph Knowledge Explorer",
            "version": __version__,
            "message": "Welcome to Reasongraph. The frontend is not built yet — run `npm run build` inside the explorer/ directory, or open the Vite dev server at http://localhost:5173.",
            "docs": "/docs",
            "health": "/health",
        })
    ...
    raise HTTPException(
        status_code=404,
        detail="Frontend not built. Run `npm run build` in reasongraph-explorer/ first."
    )

def main():
    """Server entry point.

    Binds to loopback by default; set REASONGRAPH_HOST to expose beyond
    localhost (e.g. behind a reverse proxy that terminates auth/TLS).
    """
    host = os.environ.get("REASONGRAPH_HOST", "127.0.0.1")
