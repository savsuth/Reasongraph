"""
Reasongraph CLI Entry Point

This module provides the command-line interface for the Reasongraph framework,
...
from .utils.exceptions import ReasongraphError
from .utils.logging import setup_logging

if TYPE_CHECKING:
    from .core.orchestrator import Reasongraph
...
class CLIContext:
    ...
    framework: Optional["Reasongraph"] = None
...
_ERROR_HINTS: Dict[type, str] = {
    ConnectionRefusedError: "run 'reasongraph doctor' to check backend connectivity",
    FileNotFoundError:      "check that the path exists and is readable",
    PermissionError:        "check file and directory permissions",
    ImportError:            "install the missing extras: pip install reasongraph[…]",
...
def _run_with_error_handling(action: Callable[[], None]) -> None:
    ...
    except ReasongraphError as exc:
        cause = exc.__cause__
        hint = _ERROR_HINTS.get(type(cause)) if cause else None
        _show_error_card("Reasongraph error", str(exc), hint=hint)
        raise SystemExit(1)
...
def _setup_cli_logging(
    ...
    *,
    ...
    The library logging default writes to ``reasongraph.log`` in the current
...
def _get_framework(cli_ctx: CLIContext) -> "Reasongraph":
    """Lazily initialize framework only when a command needs it."""
    if cli_ctx.framework is None:
        from .core.orchestrator import Reasongraph

        cli_ctx.framework = Reasongraph(config=cli_ctx.config.to_dict())
...
def _check_log_directory(cli_ctx: CLIContext) -> str:
    ...
    The CLI may already hold an open file handle for ``reasongraph.log``. Probe
    with a unique temporary file so Windows can check writability without
    deleting the active log file.
    """
    log_file = cli_ctx.config.get("logging.file", "reasongraph.log")
    if not log_file:
    ...
    try:
        probe_path = (
            log_dir
            / f".reasongraph-doctor-{os.getpid()}-{time.time_ns()}.tmp"
        )
...
class RichGroup(click.Group):
    ...
    def format_help(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        h.print(
            Panel(
                Text.from_markup(
                    f"[{_BRAND}]reasongraph[/{_BRAND}]  [{_DIM}]v{__version__}[/{_DIM}]\n"
                    "Knowledge Intelligence Platform"
                ),
                box=box.ROUNDED, padding=(0, 2), expand=False,
            )
        )
        h.print(f"[{_DIM}]Usage:[/{_DIM}]  reasongraph [OPTIONS] COMMAND [ARGS]...\n")
        ...
        for cmd_str, desc in [
            ("reasongraph ingest data/",    "load documents into the knowledge base"),
            ("reasongraph extract doc.pdf", "extract entities and relations"),
            ("reasongraph kg build",         "build the knowledge graph"),
            ("reasongraph reason run",       "run the reasoning engine"),
            ("reasongraph shell",            "interactive REPL"),
        ]:
...
def _show_startup(cli_ctx: CLIContext) -> None:
    """Dashboard printed when reasongraph is run with no subcommand."""
    if cli_ctx.quiet or cli_ctx.json_output:
        return
    ...
    console.print()
    console.print(
        f"  [{_DIM}]Run [bold]reasongraph --help[/bold] for all commands  •  "
        f"[bold]reasongraph shell[/bold] for interactive mode[/{_DIM}]"
    )
    console.print()
...
def main(
    ...
    """Reasongraph - Semantic Layer & Knowledge Engineering Framework"""
    try:
...
def services(ctx: click.Context) -> None:
    ...
    Subcommands will follow the spec layout::
        ...
        reasongraph services server  start|stop|status
        reasongraph services explorer start|stop|status
        reasongraph services mcp      start|stop|status
    """
    if ctx.invoked_subcommand is None:
        click.echo(ctx.get_help())
...
def info(cli_ctx: CLIContext):
    """Display information about Reasongraph."""
    ...
    def _action() -> None:
        console.print(
            Panel(
                Text.from_markup(
                    f"[{_BRAND}]Reasongraph Framework[/{_BRAND}]  v{__version__}\n"
...
def shell(ctx: click.Context) -> None:
    """Interactive REPL — run subcommands without the 'reasongraph' prefix."""
    import shlex
    ...
    if not cli_ctx.quiet:
        console.print(
            Panel(
                Text.from_markup(
                    f"[{_BRAND}]Reasongraph Shell[/{_BRAND}]  [{_DIM}]v{__version__}[/{_DIM}]\n"
...
def changelog(cli_ctx: CLIContext, local_json: bool) -> None:
    ...
    def _action() -> None:
        api = "https://api.github.com/repos/savsuth/Reasongraph/releases/latest"
        try:
        ...
        status = (
            ...
            if up_to_date
                 ...
                 f"[{_DIM}]pip install --upgrade reasongraph[/{_DIM}]"
        )
        console.print(
            Panel(
                Text.from_markup(
                    f"[{_BRAND}]Reasongraph {tag}[/{_BRAND}]\n\n"
                    f"{body}\n\n"
                    f"{status}"
                ),
...
def doctor(cli_ctx: CLIContext, local_json: bool) -> None:
    """Run a health check on all Reasongraph components and backends."""
    ...
    def _action() -> None:
        ...
        # Reasongraph version
        checks.append(("reasongraph", "ok", __version__, None))
        ...
        checks.append(_check("Graph store", _graph, hint="run 'reasongraph store list' to see configured backends"))
        ...
        checks.append(_check("Vector store", _vector, hint="pip install reasongraph[vectorstore-…]"))
        ...
        if cli_ctx.config_path:
            ...
            checks.append(("Config file", "warn", "using defaults (no --config)", "run 'reasongraph init'"))
...
def init_cmd(cli_ctx: CLIContext, force: bool) -> None:
    """Interactive wizard to create ~/.reasongraph/config.yaml."""
    cli_ctx = _require_ctx(cli_ctx)

    def _action() -> None:
        config_dir = Path.home() / ".reasongraph"
        ...
        if not cli_ctx.quiet:
            console.print(
                Panel(
                    Text.from_markup(
                        f"[{_BRAND}]Reasongraph Setup Wizard[/{_BRAND}]\n"
                        f"[{_DIM}]Creates ~/.reasongraph/config.yaml  •  press Ctrl+C to abort[/{_DIM}]"
                    ),
                    box=box.ROUNDED, padding=(0, 2), expand=False,
                )
            )
        ...
        _info(cli_ctx, "Run 'reasongraph doctor' to verify your setup.")
...
def watch_cmd(cli_ctx: CLIContext, path: str, ingestor_type: Optional[str],
    ...
    \b
    Requires: pip install reasongraph[watch]
    Examples:
      reasongraph watch ./data/contracts/
      reasongraph watch ./reports/ --patterns "*.pdf,*.docx"
    """
    try:
        ...
        raise click.ClickException(
            "watchdog is required — install it with: pip install reasongraph[watch]"
        )
...
def ingest(
    ...
    \b
    Examples:
      reasongraph ingest ./reports/q1.pdf
      reasongraph ingest ./data/ --recursive --format csv
      reasongraph ingest https://feeds.example.com/news --type feed --watch
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def parse_cmd(cli_ctx: CLIContext, file: str, parser: Optional[str], fmt: str) -> None:
    ...
    \b
    Examples:
      reasongraph parse contract.pdf | jq '.entities'
      reasongraph parse source.py --parser code --format json > parsed.json
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def split(
    ...
    \b
    Examples:
      reasongraph split report.pdf --strategy semantic --chunk-size 256
      reasongraph split data.csv --strategy table --format jsonl > chunks.jsonl
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def normalize(cli_ctx: CLIContext, input_text: str, mode: str, domain: str,
    ...
    \b
    Examples:
      reasongraph normalize "We met last Tuesday around noon." --mode temporal
      reasongraph normalize entities.json --mode entity --domain legal
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def extract(
    ...
    \b
    Examples:
      reasongraph extract "Alice signed the contract with Acme Corp."
      reasongraph extract report.pdf --mode triplets --method llm --model claude-sonnet-4-6
      cat text.txt | reasongraph extract - --mode relations
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def embed_generate(cli_ctx: CLIContext, input_path: str, model: str,
    ...
    \b
    Example:
      reasongraph embed generate ./entities.json --model sentence-transformers --output embeddings.parquet
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def embed_search(cli_ctx: CLIContext, query_text: str, store: Optional[str],
    ...
    \b
    Example:
      reasongraph embed search "CEO of Acme Corp" --store qdrant --top-k 5 --hybrid
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def embed_index(cli_ctx: CLIContext, file_path: str, store: str,
    ...
    \b
    Example:
      reasongraph embed index embeddings.parquet --store faiss --namespace production
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def deduplicate(
    ...
    \b
    Examples:
      reasongraph deduplicate --strategy hybrid --min-similarity 0.8
      reasongraph deduplicate --action merge --dry-run
      reasongraph deduplicate --action report --output duplicates.csv
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def reason_run(cli_ctx: CLIContext, engine: str, rules: Optional[str],
    ...
    \b
    Example:
      reasongraph reason run --engine rete --rules business-rules.yaml
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def reason_explain(cli_ctx: CLIContext, conclusion: str, depth: int,
    ...
    \b
    Example:
      reasongraph reason explain "Alice is-manager-of Engineering" --format markdown
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def reason_query(cli_ctx: CLIContext, query_str: str, with_inference: bool,
    ...
    \b
    Example:
      reasongraph reason query "SELECT ?x WHERE { ?x :hasRole :Manager }" --with-inference
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def reason_list(cli_ctx: CLIContext) -> None:
    ...
    try:
        import reasongraph.reasoning as _reasoning_mod
...
def decision_record(cli_ctx: CLIContext, title: str, tags: Optional[str],
    ...
    \b
    Example:
      reasongraph decision record --title "Approve vendor X" --tags "finance,vendor"
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def decision_trace(cli_ctx: CLIContext, decision_id: str, fmt: str, local_json: bool) -> None:
    ...
    \b
    Example:
      reasongraph decision trace dec_abc123 --format mermaid
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def temporal_snapshot(cli_ctx: CLIContext, at_time: str, fmt: str, local_json: bool) -> None:
    ...
    \b
    Example:
      reasongraph temporal snapshot --at "2026-01-15T09:00:00Z"
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def temporal_history(cli_ctx: CLIContext, entity_id: str, since: Optional[str],
    ...
    \b
    Example:
      reasongraph temporal history entity_alice --since 2025-01-01 --format table
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def provenance_lineage(cli_ctx: CLIContext, entity_id: str, depth: int, local_json: bool) -> None:
    ...
    \b
    Example:
      reasongraph provenance lineage entity_alice --depth 3
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def provenance_audit(cli_ctx: CLIContext, since: Optional[str], fmt: str,
    ...
    \b
    Example:
      reasongraph provenance audit --since 2026-01-01 --format csv --output audit.csv
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def provenance_export(cli_ctx: CLIContext, fmt: str, base_uri: Optional[str],
    ...
    \b
    Example:
      reasongraph provenance export --format turtle --output prov.ttl
      reasongraph provenance export --base-uri https://example.org/kg# --output prov.ttl
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def provenance_invalidate(cli_ctx: CLIContext, entity_id: str, agent_id: str,
    ...
    \b
    Example:
      reasongraph provenance invalidate entity_alice --by reviewer_jane --reason "Source retracted"
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def provenance_verify_chain(cli_ctx: CLIContext, local_json: bool) -> None:
    ...
    \b
    Example:
      reasongraph provenance verify-chain
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def provenance_descendants(cli_ctx: CLIContext, entity_id: str, depth: Optional[int],
    ...
    \b
    Example:
      reasongraph provenance descendants entity_alice --depth 3
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def validate_shacl(cli_ctx: CLIContext, shapes: Optional[str], strictness: str,
    ...
    \b
    Example:
      reasongraph validate shacl --strictness strict --report report.json
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def validate_conflicts(cli_ctx: CLIContext, strategy: str, fmt: str, local_json: bool) -> None:
    ...
    \b
    Example:
      reasongraph validate conflicts --format json | jq '.conflicts | length'
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def ontology_generate(cli_ctx: CLIContext, domain: Optional[str], output: Optional[str],
    ...
    \b
    Example:
      reasongraph ontology generate --domain healthcare --output healthcare.ttl
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def ontology_import(cli_ctx: CLIContext, source: str, fmt: Optional[str],
    ...
    \b
    Example:
      reasongraph ontology import schema.org.ttl --format turtle
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def ontology_align(cli_ctx: CLIContext, source: str, target: str, strategy: str,
    ...
    \b
    Example:
      reasongraph ontology align --source mine.ttl --target schema.ttl --strategy semantic
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def export(
    ...
    \b
    Examples:
      reasongraph export --format turtle --output graph.ttl
      reasongraph export --format parquet --with-provenance --output graph.parquet
      reasongraph export --format csv --filter "type:Person" --output persons.csv
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def pipeline_init(cli_ctx: CLIContext, template: str, output: str, local_dry: bool) -> None:
    ...
    \b
    Example:
      reasongraph pipeline init --template ingest-extract-kg --output pipeline.yaml
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def pipeline_run(cli_ctx: CLIContext, pipeline_file: str, parallel: int,
    ...
    \b
    Example:
      reasongraph pipeline run pipeline.yaml --parallel 8
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def store_migrate(cli_ctx: CLIContext, from_backend: str, to_backend: str,
    ...
    \b
    Example:
      reasongraph store migrate --from faiss --to qdrant --namespace production --dry-run
    """
    cli_ctx = _require_ctx(cli_ctx)
    ...
    def _action() -> None:
        ...
        raise click.ClickException(
            ...
            "  reasongraph export --format parquet --output dump.parquet\n"
            f"  reasongraph embed index dump.parquet --store {to_backend}"
...
def backup(ctx: click.Context) -> None:
    """Back up and restore all Reasongraph data."""
    if ctx.invoked_subcommand is None:
        click.echo(ctx.get_help())
...
def _collect_backup_sources(
    ...
    if not strip_config and include in ("config", "all"):
        candidates: List[str] = [c for c in [config_path, "reasongraph.yaml", "reasongraph.yml"] if c]
        for c in candidates:
            p = Path(c)
            if p.is_file():
                sources.append(("config/reasongraph.yaml", p))
                break
    ...
    def _action() -> None:
        ...
        for store_type, cfg_key in section_keys.items():
            ...
            if backend in cloud_backends:
                method = "use `reasongraph export` (cloud-managed)"
...
@click.option("--strip-config", is_flag=True, default=False,
              help="Exclude reasongraph.yaml to avoid storing credentials at rest.")
@click.option("--dry-run", "local_dry", is_flag=True, default=False)
...
def backup_create(
    ...
    \b
    Example:
      reasongraph backup create ./backups/reasongraph-2026-05-25.tar.gz --compress --encrypt
    """
    cli_ctx = _require_ctx(cli_ctx)
    ...
    def _action() -> None:
        ...
        if not strip_config and not encrypt and passphrase is None:
            if not quiet:
                console.print(
                    f"[{_WARN_STY}] ⚠[/{_WARN_STY}] backup includes reasongraph.yaml which may contain "
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            raw_tar = Path(tmp) / "reasongraph-backup.tar"
            with tarfile.open(str(raw_tar), "w") as tar:
                tar.add(str(tmp_path / "backup_manifest.json"),
                        arcname="reasongraph-backup/backup_manifest.json")
                for arcname, local_path in sources:
                    tar.add(str(local_path), arcname=f"reasongraph-backup/{arcname}")

            # Compress (gzip) if requested.
            if compress:
                import gzip
                compressed = Path(tmp) / "reasongraph-backup.tar.gz"
            ...
            if encrypt and passphrase:
                ...
                enc_path = Path(tmp) / "reasongraph-backup.enc"
...
def backup_sync(cli_ctx: CLIContext, destination: str, include: str,
    ...
    \b
    Example:
      reasongraph backup sync /mnt/external/reasongraph-backup
    """
    cli_ctx = _require_ctx(cli_ctx)
    quiet = cli_ctx.quiet or local_quiet
...
def backup_restore(cli_ctx: CLIContext, source: str, local_dry: bool) -> None:
    ...
    \b
    Example:
      reasongraph backup restore ./backups/reasongraph-2026-05-25.tar.gz --dry-run
    """
    cli_ctx = _require_ctx(cli_ctx)
            ...
            if _tf.is_tarfile(str(work_path)):
                ...
                with _tf.open(str(work_path), "r:*") as tar:
                    ...
                    for member in tar.getmembers():
                        # Strip the leading "reasongraph-backup/" prefix
                        member.name = member.name.replace("reasongraph-backup/", "", 1)
                        if not member.name:
                            continue

                        # Reject members whose resolved path escapes the
                        # restore root (path traversal / absolute paths),
                        # regardless of the "reasongraph-backup/" prefix.
...
def backup_schedule(cli_ctx: CLIContext, dest: str, freq: str, encrypt: bool) -> None:
    ...
    \b
    Example:
      reasongraph backup schedule --dest /mnt/external/reasongraph --freq daily --encrypt
    """
    cli_ctx = _require_ctx(cli_ctx)
    ...
    snippet = (
        f"{cron_time} reasongraph backup sync {dest}{enc_flag} --quiet"
    )
    if cli_ctx.json_output:
        _jecho({"cron": snippet})
    else:
...
def _pid_file(name: str) -> Path:
    return Path.home() / ".reasongraph" / f"{name}.pid"
...
def server_start(cli_ctx: CLIContext, port: int, workers: int, reload: bool, host: str) -> None:
    ...
    \b
    Example:
      reasongraph server start --port 8000 --workers 4
    """
    cli_ctx = _require_ctx(cli_ctx)
    ...
    if host not in _LOOPBACK_HOSTS:
        console.print(
            ...
            "the server to the network. Set REASONGRAPH_API_KEY before doing this "
            "in any reachable environment — without it, protected routes refuse "
            "all requests (503), and with REASONGRAPH_ALLOW_ANONYMOUS=true they are "
            "wide open."
        )
    ...
    def _action() -> None:
        ...
        cmd = [
            sys.executable, "-m", "uvicorn", "reasongraph.server:app",
...
def explorer_start(cli_ctx: CLIContext, port: int, api_url: str,
    ...
    \b
    Example:
      reasongraph explorer start --port 5173 --api-url http://localhost:8000
    """
    cli_ctx = _require_ctx(cli_ctx)

    def _action() -> None:
        import subprocess as sp
        cmd = [sys.executable, "-m", "reasongraph.explorer", "--port", str(port)]
        if graph:
            cmd += ["--graph", graph]
        env = os.environ.copy()
        env["REASONGRAPH_API_URL"] = api_url
...
def mcp_start(cli_ctx: CLIContext, transport: str, port: int) -> None:
    ...
    \b
    Example:
      reasongraph mcp start --transport http --port 3000
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def mcp_call(cli_ctx: CLIContext, tool_name: str, args: str, local_json: bool) -> None:
    ...
    \b
    Example:
      reasongraph mcp call extract_entities --args '{"text": "Alice works at Acme Corp."}'
    """
    cli_ctx = _require_ctx(cli_ctx)
...
def completion(shell: str) -> None:
    ...
    \b
    Install:
      reasongraph completion bash        >> ~/.bashrc
      reasongraph completion zsh         >> ~/.zshrc
      reasongraph completion fish        > ~/.config/fish/completions/reasongraph.fish
      reasongraph completion powershell  >> $PROFILE
    """
    shell_map = {
        "bash": ("bash", "~/.bashrc"),
        "zsh": ("zsh", "~/.zshrc"),
        "fish": ("fish", "~/.config/fish/completions/reasongraph.fish"),
        "powershell": ("powershell", "$PROFILE"),
    }
    click_shell, install_path = shell_map[shell]
    env_var = "_REASONGRAPH_COMPLETE"
    try:
        from click.shell_completion import ShellComplete
        complete = ShellComplete(main, ctx_args={}, prog_name="reasongraph",
        ...
        click.echo(
            ...
            f'eval "$({env_var}={click_shell}_source reasongraph)"'
        )
