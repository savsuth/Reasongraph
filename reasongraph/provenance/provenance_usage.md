...
The Reasongraph provenance module provides W3C PROV-O compliant tracking for knowledge graphs, enabling complete end-to-end lineage from source documents to query responses. This module consolidates and enhances provenance tracking from:
...
The provenance module is included with Reasongraph. No additional installation required.
...
- **prov:Agent** - Who/what performed the activity (Reasongraph, user, model)
...
```python
from reasongraph.provenance import SourceReference
...
All Reasongraph modules now have provenance-enabled versions. Simply add `provenance=True` to enable tracking.

### Semantic Extract (NER, Relations, Events)

```python
from reasongraph.semantic_extract.semantic_extract_provenance import (
...
```python
from reasongraph.llms.llms_provenance import (
...
```python
from reasongraph.pipeline.pipeline_provenance import PipelineWithProvenance
from reasongraph.pipeline import PipelineBuilder
...
```python
from reasongraph.context.context_provenance import ContextManagerWithProvenance
...
```python
from reasongraph.ingest.ingest_provenance import PDFIngestorWithProvenance
...
```python
from reasongraph.embeddings.embeddings_provenance import EmbeddingGeneratorWithProvenance
...
```python
from reasongraph.graph_store.graph_store_provenance import GraphStoreWithProvenance
...
```python
from reasongraph.vector_store.vector_store_provenance import VectorStoreWithProvenance
...
```python
from reasongraph.triplet_store.triplet_store_provenance import TripletStoreWithProvenance
...
```python
# Reasoning
from reasongraph.reasoning.reasoning_provenance import ReasoningEngineWithProvenance
reasoner = ReasoningEngineWithProvenance(provenance=True)

# Conflicts
from reasongraph.conflicts.conflicts_provenance import SourceTrackerWithUnifiedBackend
tracker = SourceTrackerWithUnifiedBackend()

# Deduplication
from reasongraph.deduplication.deduplication_provenance import DeduplicatorWithProvenance
dedup = DeduplicatorWithProvenance(provenance=True)

# Export
from reasongraph.export.export_provenance import ExporterWithProvenance
exporter = ExporterWithProvenance(provenance=True)

# Parse
from reasongraph.parse.parse_provenance import ParserWithProvenance
parser = ParserWithProvenance(provenance=True)

# Normalize
from reasongraph.normalize.normalize_provenance import NormalizerWithProvenance
normalizer = NormalizerWithProvenance(provenance=True)

# Ontology
from reasongraph.ontology.ontology_provenance import OntologyManagerWithProvenance
ontology = OntologyManagerWithProvenance(provenance=True)

# Visualization
from reasongraph.visualization.visualization_provenance import VisualizerWithProvenance
...
from reasongraph.provenance import ProvenanceManager
from reasongraph.ingest.ingest_provenance import PDFIngestorWithProvenance
from reasongraph.semantic_extract.semantic_extract_provenance import NERExtractorWithProvenance
from reasongraph.llms.llms_provenance import GroqLLMWithProvenance
from reasongraph.graph_store.graph_store_provenance import GraphStoreWithProvenance
...
```python
from reasongraph.provenance import ProvenanceManager
from reasongraph.provenance.bridge_axiom import BridgeAxiom
...
```python
from reasongraph.provenance.bridge_axiom import create_translation_chain, BridgeAxiom
...
from reasongraph.provenance.bridge_axiom import trace_translation_chain
...
```python
from reasongraph.provenance import ProvenanceManager, InMemoryStorage
...
```python
from reasongraph.provenance import ProvenanceManager, SQLiteStorage
...
```python
from reasongraph.provenance import ProvenanceStorage
...
```python
from reasongraph.kg import ProvenanceTracker
...
```python
from reasongraph.split import ProvenanceTracker
from reasongraph.split.semantic_chunker import Chunk
...
```python
from reasongraph.provenance import compute_checksum, verify_checksum
...
**Important:** This module provides the *technical capabilities* for compliance. Organizations must implement additional policies, procedures, validation, and controls to meet specific regulatory requirements. Reasongraph does not provide regulatory certification or legal compliance guarantees.
