"""
OpenClaw ↔ Reasongraph bridge
============================

Two integration paths:

1. **MCP (recommended)** — ``OpenClawMCPConfig`` emits the ``mcporter.json``
   snippet that wires Reasongraph's MCP server into the OpenClaw Gateway.
   All 12 Reasongraph MCP tools become native OpenClaw agent tools with no
   extra code.

2. **REST** — ``OpenClawKGTool`` is a plain Python class that calls the
   Reasongraph REST API (port 8000) and can be registered as an OpenClaw
...
class OpenClawMCPConfig:
    """
    Generates the ``mcporter.json`` entry needed to connect Reasongraph's MCP
    server to the OpenClaw Gateway.

    Parameters
    ----------
    server_command:
        Shell command used to launch the Reasongraph MCP server.
        Defaults to ``"python -m reasongraph.mcp_server"``.
    transport:
        MCP transport protocol.  OpenClaw supports ``"stdio"`` (default)
        and ``"sse"``.
    name:
        Key used in ``mcporter.json``.  Defaults to ``"reasongraph"``.
    ...
    def __init__(
        self,
        server_command: str = "python -m reasongraph.mcp_server",
        transport: str = "stdio",
        name: str = "reasongraph",
    ) -> None:
...
class OpenClawKGTool:
    """
    A Reasongraph knowledge-graph tool callable from an OpenClaw agent.

    Wraps the Reasongraph REST API so that an OpenClaw agent configured with
    ...
    ----------
    base_url:
        Base URL of the running Reasongraph REST server.
    ...
    dependency under ``reasongraph[openclaw]``; install it with::

        pip install reasongraph[openclaw]
    """

    TOOL_NAME = "reasongraph_kg"
    TOOL_DESCRIPTION = (
        "Reasongraph knowledge-graph tool. "
        ...
        if self._session is None:
            try:
                ...
                raise ImportError(
                    ...
                    "Install it with: pip install reasongraph[openclaw]"
                ) from exc
    ...
    def run_reasoning(self, rules: List[str], facts: List[str]) -> Dict[str, Any]:
        """Run the Reasongraph forward-chaining reasoner."""
