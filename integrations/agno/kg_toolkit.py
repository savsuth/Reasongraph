...
reasoning loop.  Backed by Reasongraph's ``NERExtractor``, ``RelationExtractor``,
...
class AgnoKGToolkit(_ToolkitBase):  # type: ignore[misc]
    """
    Agno Toolkit that surfaces Reasongraph's KG pipeline as agent tools.
    ...
    relation_extractor:
        ...
        created when ``None``.
    reasoner:
        A ``reasongraph.reasoning.Reasoner`` instance; auto-created when
        ``None``.
    context:
    ...
    def export_subgraph(
        ...
        """
        try:
            from reasongraph.export import RDFExporter  # lazy import
