"""
AgnoKnowledgeGraph — Relational agent knowledge backed by Reasongraph's KG pipeline.
...
Ingested documents pass through the full Reasongraph extraction pipeline:
...
class AgnoKnowledgeGraph(_KnowledgeBase):  # type: ignore[misc]
    """
    Relational agent knowledge store backed by Reasongraph's KG pipeline.

    Parameters
    ----------
    graph_builder:
        A ``reasongraph.kg.GraphBuilder`` instance.  Created automatically if
        ``None``.
    ner_extractor:
        A ``reasongraph.semantic_extract.NERExtractor`` instance.  Created
        automatically if ``None``.
    relation_extractor:
        A ``reasongraph.semantic_extract.RelationExtractor`` instance.  Created
        automatically if ``None``.
    context_graph:
        An existing ``reasongraph.context.ContextGraph`` to use as the backing
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Lazy imports to keep reasongraph core optional at import time
