...
Implements Agno's ``MemoryDb`` protocol backed by Reasongraph's ``AgentContext``,
...
class AgnoContextStore(_MemoryDbBase):  # type: ignore[misc]
    ...
    ----------
    vector_store:
        A ``reasongraph.vector_store.VectorStore`` instance (or ``None`` to use
        an in-memory FAISS store created automatically).
    knowledge_graph:
        A ``reasongraph.context.ContextGraph`` instance (or ``None`` for a fresh
        in-memory graph).
    decision_tracking:
    ...
    def __init__(
        ...
        **agent_context_kwargs: Any,
        ...
        # ------------------------------------------------------------------
        from reasongraph.context import AgentContext, ContextGraph  # lazy import
        from reasongraph.vector_store import VectorStore  # lazy import
    ...
    # ------------------------------------------------------------------
    # Extended Reasongraph API (usable from application code directly)
    # ------------------------------------------------------------------
