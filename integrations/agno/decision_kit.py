...
Exposes Reasongraph's decision intelligence as native Agno tools so that agents
...
class AgnoDecisionKit(_ToolkitBase):  # type: ignore[misc]
    """
    Agno Toolkit that surfaces Reasongraph's decision intelligence as agent tools.

    Parameters
    ----------
    context:
        A ``reasongraph.context.AgentContext`` (or ``AgentContext``-compatible
