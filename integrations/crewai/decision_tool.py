"""
ReasongraphDecisionTool — a CrewAI ``BaseTool`` exposing Reasongraph's decision
...
# Input schema
# ---------------------------------------------------------------------------
class ReasongraphDecisionToolInput(BaseModel):
    """
    Input schema for ``ReasongraphDecisionTool``.
...
# ---------------------------------------------------------------------------
class ReasongraphDecisionTool(_BaseTool):  # type: ignore[misc]
    """
    CrewAI tool that surfaces Reasongraph's decision intelligence as agent actions.

    Parameters
    ----------
    context:
        A ``reasongraph.context.AgentContext`` (or compatible object exposing
    ...
    args_schema: Type[BaseModel] = ReasongraphDecisionToolInput
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger.info("ReasongraphDecisionTool initialised (crewai=%s)", CREWAI_AVAILABLE)
    ...
    def _ensure_defaults(self) -> None:
        ...
        if self.context is None:
            ...
            if self.had_live_state:
                ...
                logger.warning(
                    "ReasongraphDecisionTool: the live decision context was lost "
                ...
                logger.warning(
                    "ReasongraphDecisionTool created a fresh in-memory "
    ...
    def _eval_rule(self, rule: str, data: Dict[str, Any]) -> bool:
        ...
        action — it is intentionally independent of Reasongraph's policy engine
