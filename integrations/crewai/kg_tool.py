"""
ReasongraphKGTool — a CrewAI ``BaseTool`` exposing Reasongraph's knowledge-graph
...
# Input schema
# ---------------------------------------------------------------------------
class ReasongraphKGToolInput(BaseModel):
    """
    Input schema for ``ReasongraphKGTool``.
...
# ReasongraphKGTool
# ---------------------------------------------------------------------------
class ReasongraphKGTool(_BaseTool):  # type: ignore[misc]
    """
    CrewAI tool that surfaces Reasongraph's KG pipeline as agent actions.

    Parameters
    ----------
    graph:
        A ``reasongraph.context.ContextGraph`` to read/write.  A fresh in-memory
        graph is used when ``None``.
    ner_extractor:
        A ``reasongraph.semantic_extract.NERExtractor`` instance; auto-created
        when ``None``.
    relation_extractor:
        A ``reasongraph.semantic_extract.RelationExtractor`` instance; auto-
        created when ``None``.
    """
    ...
    args_schema: Type[BaseModel] = ReasongraphKGToolInput
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger.info("ReasongraphKGTool initialised (crewai=%s)", CREWAI_AVAILABLE)
    ...
    def _ensure_defaults(self) -> None:
        ...
        if self.graph is None:
            ...
            if self.had_live_state:
                ...
                logger.warning(
                    "ReasongraphKGTool: the live graph was lost during "
                ...
                logger.warning(
                    "ReasongraphKGTool created a fresh in-memory ContextGraph — "
    ...
    # ------------------------------------------------------------------
    # Entity/relation field access (handles both Reasongraph dataclasses and
