"""
ReasongraphKnowledgeSource — expose a Reasongraph ``ContextGraph`` as a CrewAI
knowledge source.
...
full Reasongraph API, but cannot be passed to a ``Crew``.
"""
...
from reasongraph.utils.logging import get_logger
...
class ReasongraphKnowledgeSource(_BaseKnowledgeSource):  # type: ignore[misc]
    """
    CrewAI knowledge source backed by a Reasongraph ``ContextGraph``.
    ...
    ----------
    graph:
        A ``reasongraph.context.ContextGraph`` to expose.  A fresh in-memory
        graph is created when ``None``.
    name:
        Source name.  Defaults to ``"reasongraph_knowledge_graph"``.
    chunk_size:
    ...
    name: str = "reasongraph_knowledge_graph"
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if CREWAI_AVAILABLE:
            ...
            super().__init__(
                graph=graph,
                name=name or "reasongraph_knowledge_graph",
            ...
            super().__init__()
            self.graph = graph
            self.name = name or "reasongraph_knowledge_graph"
            self.chunk_size = int(chunk_size)
            self.chunk_overlap = int(chunk_overlap)

        logger.info(
            "ReasongraphKnowledgeSource initialised (crewai=%s, chunk_size=%d)",
            CREWAI_AVAILABLE,
            self.chunk_size,
        )
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if self.graph is None:
            ...
            if self.had_live_state:
                ...
                logger.warning(
                    "ReasongraphKnowledgeSource: the live graph was lost during "
                ...
                logger.warning(
                    "ReasongraphKnowledgeSource created a fresh in-memory "
    ...
    def load_content(self) -> Dict[str, str]:
        ...
        try:
            ...
            logger.warning(
                "ReasongraphKnowledgeSource.load_content (nodes) failed: %s", exc
            )
        ...
        try:
            ...
            logger.warning(
                "ReasongraphKnowledgeSource.load_content (edges) failed: %s", exc
            )
    ...
    def validate_content(self) -> Any:
        ...
        """
        if self.graph is None:
            raise ValueError("ReasongraphKnowledgeSource requires a ContextGraph.")
        return True
    ...
    def _chunk(self, text: str) -> List[str]:
        ...
        if helper is not None:
            try:
                ...
                logger.debug(
                    "ReasongraphKnowledgeSource._chunk_text failed, falling back: %s", exc
                )
    ...
    def add(self) -> None:
        ...
        if not content:
            logger.debug("ReasongraphKnowledgeSource.add: empty graph — nothing to store")
            return
        ...
        if save is not None:
            if getattr(self, "storage", None) is None:
                logger.debug(
                    "ReasongraphKnowledgeSource.add: storage not wired — "
                ...
                try:
                    save()
                    logger.info(
                        "ReasongraphKnowledgeSource.add: stored %d chunks", len(chunks)
                    )
                    return
                except Exception as exc:
                    logger.error(
                        "ReasongraphKnowledgeSource.add: storage save FAILED (%s) — "
        ...
        logger.info(
            "ReasongraphKnowledgeSource.add: %d chunks ready in memory", len(chunks)
        )
