"""
Capability Gap Analysis with Reasongraph Context Graphs

This example mirrors the military capability-gap notebook as a runnable Python script.
It uses Reasongraph modules and classes across ingestion, parsing, ontology handling,
...
from reasongraph.change_management import VersionManager
from reasongraph.conflicts import detect_conflicts, resolve_conflicts, voting
from reasongraph.context import (
    AgentContext,
    ContextGraph,
    Decision,
    Policy,
...
from reasongraph.ingest import FileIngestor, OntologyIngestor, ingest_web
...
from reasongraph.ontology import OntologyEvaluator, ingest_ontology
from reasongraph.parse import (
    DOCLING_AVAILABLE,
    DocumentParser,
    DoclingParser,
    PDFParser,
...
def main() -> None:
    ...
    # -------------------------------------------------------------------------
    # 1) Ingestion (Reasongraph ingest)
    # -------------------------------------------------------------------------
    ...
    # Web source ingestion through Reasongraph's `ingest_web` method wrapper.
    web_sources = [
    ...
    # -------------------------------------------------------------------------
    # 2) Ontology introspection + evaluation (Reasongraph ontology)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 5) Split + pipeline declaration (Reasongraph split + pipeline)
    # -------------------------------------------------------------------------
    ...
    # Logical orchestration path declared with Reasongraph PipelineBuilder.
    pipeline = (
        PipelineBuilder()
    ...
    # -------------------------------------------------------------------------
    # 6) Normalization (Reasongraph normalize)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 7) Semantic extraction (Reasongraph semantic_extract)
    # -------------------------------------------------------------------------
    # Initialize Reasongraph extractors for entities, relations, events, triplets.
    ...
    # -------------------------------------------------------------------------
    # 8) Quality controls + KG analytics (Reasongraph kg + conflicts)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 9) Context graph + decision traces (Reasongraph context)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 10) Rule-based reasoning (Reasongraph reasoning)
    # -------------------------------------------------------------------------
    reasoner = Reasoner()
    ...
    # -------------------------------------------------------------------------
    # 11) Versioning + provenance (Reasongraph change_management + provenance)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 12) Export + visualization (Reasongraph export + visualization)
    # -------------------------------------------------------------------------
