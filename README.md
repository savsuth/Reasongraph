...
<img src="docs/assets/img/reasongraph-banner.svg" alt="Reasongraph" width="820"/>

### Graph-Native Infrastructure for Context and Accountable AI Systems

#### *Palantir for AI Agents*
...
> **System-level explainability, not foundation-model explainability.** Reasongraph does not expose or reconstruct what happens *inside* the LLM — its internal reasoning or chain-of-thought stays opaque, as it does for any external system. Reasongraph explains what's *outside* the model: the context and data fed in, the decision produced, its provenance, relevant relationships, applied policies, and the full execution trail.
...
**[Quick Start](#quick-start)** &nbsp;·&nbsp; **[Architecture](#architecture)** &nbsp;·&nbsp; **[What You Get](#what-reasongraph-gives-you)** &nbsp;·&nbsp; **[Decision Intelligence](#decision-intelligence)** &nbsp;·&nbsp; **[Context Graphs](#context-graphs)** &nbsp;·&nbsp; **[Recipe: Audit Trail](#recipe-audit-trail-for-a-regulated-decision)** &nbsp;·&nbsp; **[Module Reference](#module-reference)** &nbsp;·&nbsp; **[Integrations](#integrations)** &nbsp;·&nbsp; **[CLI](#cli)** &nbsp;·&nbsp; **[Performance](#performance)** &nbsp;·&nbsp; **[Install](#installation)**

---

## What Reasongraph Gives You
...
reasongraph doctor
# Python 3.11.9         pass
# reasongraph 0.6.5       pass
# faiss vector store    pass
# Config file           pass    ~/.reasongraph/config.yaml
```
...
Reasongraph is a real end-to-end pipeline, not a single library with a marketing name. Every stage below is a shipping module, independently importable:
...
```python
from reasongraph.context import ContextGraph, AgentContext
from reasongraph.vector_store import VectorStore
...
```python
from reasongraph.context import ContextGraph
from reasongraph.provenance import ProvenanceManager
from reasongraph.export import RDFExporter
...
| Module | What it does |
| --- | --- |
| [`reasongraph.ingest`](#reasongraphingest-multi-source-ingestion) | Files, web, databases, APIs, streams, email, Git, Parquet, Databricks, Snowflake, MCP |
| [`reasongraph.semantic_extract`](#reasongraphsemantic_extract-ner-relations-events-triplets) | NER, relation extraction, event detection, triplet generation |
| [`reasongraph.kg`](#reasongraphkg-knowledge-graph-construction--analysis) | Graph construction, centrality, communities, link prediction |
| [`reasongraph.reasoning`](#reasongraphreasoning-forward-chaining-rete-datalog-sparql) | Forward chaining, Rete, Datalog, SPARQL, fully explainable |
| [`reasongraph.vector_store`](#reasongraphvector_store-hybrid--filtered-semantic-search) | FAISS, Qdrant, Weaviate, Milvus, Pinecone, PgVector, hybrid search |
| [`reasongraph.split`](#reasongraphsplit-graphrag-native-document-chunking) | Entity-aware, relation-aware, ontology-aware chunking for GraphRAG |
| [`reasongraph.provenance`](#reasongraphprovenance-w3c-prov-o-lineage) | W3C PROV-O lineage on every fact |
| [`reasongraph.ontology`](#reasongraphontology-owl-generation-shacl-validation) | OWL generation, SHACL validation, SKOS vocabularies |
| [`reasongraph.conflicts`](#reasongraphconflicts-conflict-detection--resolution) | Detect and resolve conflicting facts across sources |
| [`reasongraph.deduplication`](#reasongraphdeduplication-entity-resolution-at-scale) | Entity resolution at scale |
| [`reasongraph.normalize`](#reasongraphnormalize-data-normalization--cleaning) | Text, entity, date, and number normalization; dataset cleaning |
| [`reasongraph.pipeline`](#reasongraphpipeline-pipeline-dsl) | Declarative, parallel pipeline DSL for ingest → extract → build → export |
| [`reasongraph.export`](#reasongraphexport-rdf-owl-parquet-cypher-json-ld) | RDF, OWL, Parquet, Cypher, JSON-LD |
| [`reasongraph.visualization`](#reasongraphvisualization-interactive-graph-workbench) | Force-directed graphs, ontology hierarchies, temporal dashboards |
...
<details>
<summary><b><code>reasongraph.ingest</code></b>: Multi-Source Ingestion</summary>
<a id="reasongraphingest-multi-source-ingestion"></a>
...
```python
from reasongraph.ingest import FileIngestor, WebIngestor, ParquetIngestor, DBIngestor
...
from reasongraph.ingest import DatabricksIngestor, SnowflakeIngestor

# pip install "reasongraph[db-databricks]"
...
# pip install reasongraph[db-snowflake]
...
DuckDB, Elasticsearch, Google Drive, HuggingFace, MongoDB, and Pandas ingestion also ship (`DuckDBIngestor`, `ElasticIngestor`, `GDriveIngestor`, `HuggingFaceIngestor`, `MongoIngestor`, `PandasIngestor`) but aren't re-exported from the top-level `reasongraph.ingest` namespace yet — import them directly: `from reasongraph.ingest.duckdb_ingestor import DuckDBIngestor`.

</details>

<details>
<summary><b><code>reasongraph.semantic_extract</code></b>: NER, Relations, Events, Triplets</summary>
<a id="reasongraphsemantic_extract-ner-relations-events-triplets"></a>
...
<details>
<summary><b><code>reasongraph.kg</code></b>: Knowledge Graph Construction & Analysis</summary>
<a id="reasongraphkg-knowledge-graph-construction--analysis"></a>

Build a production knowledge graph from documents and run graph algorithms over it.

```python
from reasongraph.ingest import FileIngestor
from reasongraph.kg import (
    GraphBuilder,
    GraphAnalyzer,
...
<details>
<summary><b><code>reasongraph.reasoning</code></b>: Forward Chaining, Rete, Datalog, SPARQL</summary>
<a id="reasongraphreasoning-forward-chaining-rete-datalog-sparql"></a>

Run explainable rule-based inference, not a black box.

```python
from reasongraph.reasoning import ReteEngine, Rule, Fact, RuleType
...
from reasongraph.reasoning import DatalogReasoner
...
from reasongraph.reasoning import ExplanationGenerator, Reasoner
...
<details>
<summary><b><code>reasongraph.vector_store</code></b>: Hybrid & Filtered Semantic Search</summary>
<a id="reasongraphvector_store-hybrid--filtered-semantic-search"></a>

Drop-in vector store with multiple backends, hybrid search, and decision-aware retrieval.

```python
from reasongraph.vector_store import VectorStore, HybridSearch
...
<details>
<summary><b><code>reasongraph.split</code></b>: GraphRAG-Native Document Chunking</summary>
<a id="reasongraphsplit-graphrag-native-document-chunking"></a>
...
```python
from reasongraph.split import TextSplitter, EntityAwareChunker, RelationAwareChunker
...
<details>
<summary><b><code>reasongraph.provenance</code></b>: W3C PROV-O Lineage</summary>
<a id="reasongraphprovenance-w3c-prov-o-lineage"></a>
...
<details>
<summary><b><code>reasongraph.ontology</code></b>: OWL Generation, SHACL Validation</summary>
<a id="reasongraphontology-owl-generation-shacl-validation"></a>

Generate ontologies from data, validate shapes, and manage your vocabulary.

```python
from reasongraph.ontology import OntologyGenerator, OntologyValidator
...
gen       = OntologyGenerator(base_uri="https://reasongraph.dev/ontology/")
...
<details>
<summary><b><code>reasongraph.conflicts</code></b>: Conflict Detection & Resolution</summary>
<a id="reasongraphconflicts-conflict-detection--resolution"></a>

Detect and resolve conflicting facts from multiple sources before they corrupt your knowledge base.

```python
from reasongraph.conflicts import ConflictDetector, ConflictResolver, SourceTracker
...
<details>
<summary><b><code>reasongraph.deduplication</code></b>: Entity Resolution at Scale</summary>
<a id="reasongraphdeduplication-entity-resolution-at-scale"></a>

Block, cluster, and merge duplicates with semantic similarity.

```python
from reasongraph.deduplication import DuplicateDetector, EntityMerger
...
<details>
<summary><b><code>reasongraph.normalize</code></b>: Data Normalization & Cleaning</summary>
<a id="reasongraphnormalize-data-normalization--cleaning"></a>

Standardize text, entities, dates, numbers, and encodings before building your knowledge graph.

```python
from reasongraph.normalize import (
    TextNormalizer,
    EntityNormalizer,
    DateNormalizer,
    NumberNormalizer,
...
<details>
<summary><b><code>reasongraph.pipeline</code></b>: Pipeline DSL</summary>
<a id="reasongraphpipeline-pipeline-dsl"></a>

Compose ingestion, extraction, and graph-building into a declarative, parallel pipeline.

```python
from reasongraph.pipeline import PipelineBuilder, ExecutionEngine
...
<details>
<summary><b><code>reasongraph.export</code></b>: RDF, OWL, Parquet, Cypher, JSON-LD</summary>
<a id="reasongraphexport-rdf-owl-parquet-cypher-json-ld"></a>

Export to any format required by regulators, graph databases, or downstream systems.

```python
from reasongraph.export import (
    RDFExporter,
    JSONExporter,
    ParquetExporter,
    LPGExporter,
...
<details>
<summary><b><code>reasongraph.visualization</code></b>: Interactive Graph Workbench</summary>
<a id="reasongraphvisualization-interactive-graph-workbench"></a>

Render force-directed graphs, community maps, ontology hierarchies, and temporal dashboards.

```python
from reasongraph.visualization import (
    KGVisualizer,
    OntologyVisualizer,
    EmbeddingVisualizer,
    TemporalVisualizer,
...
[Runnable notebooks in the cookbook](cookbook/), each self-contained and runnable in under 5 minutes
...
```python
from reasongraph.ingest import FileIngestor
from reasongraph.split import TextSplitter
from reasongraph.semantic_extract import NamedEntityRecognizer, RelationExtractor
from reasongraph.kg import GraphBuilder
from reasongraph.vector_store import VectorStore, HybridSearch
from reasongraph.context import AgentContext
...
Same condition-matcher caveat as [above](#reasongraphreasoning-forward-chaining-rete-datalog-sparql) applies — validate against your rule set before production use.
...
| **LLM Providers** | **All already supported today:** OpenAI (GPT-4o, o1, o3) · Anthropic (Claude) · Google Gemini · Mistral · Meta Llama · Groq · Cohere · Azure OpenAI · AWS Bedrock · Ollama · DeepSeek · Perplexity · Together AI · Fireworks AI · Replicate · HuggingFace · via `reasongraph.llms` and LiteLLM |
...
```bash
pip install reasongraph
reasongraph        # startup dashboard
reasongraph doctor # health check
reasongraph --help # full grouped command reference
```

Start with `reasongraph`, verify with `doctor`, build a graph, and explore the command groups from one terminal.
...
<sub>First-class · <code>pip install reasongraph[agno]</code></sub>
</td>
<td align="center" width="12.5%">
...
<sub>First-class · <code>pip install reasongraph[crewai]</code></sub>
</td>
</tr>
<tr>
...
{
  "mcpServers": {
    "reasongraph": { "command": "python", "args": ["-m", "reasongraph.mcp_server"] }
  }
}
```
...
```bash
# Start the backend
python -m reasongraph.server   # port 8000
...
For dev-server setup: **[explorer/README.md: Local Setup Guide](explorer/README.md)**
...
pip install reasongraph[all]      # everything
```

```bash
pip install reasongraph[agno]                 # Agno multi-agent integration
pip install reasongraph[crewai]               # CrewAI integration
pip install reasongraph[llm-litellm]          # OpenAI, Anthropic, Gemini, Mistral, Llama, Groq, Cohere, Bedrock, Ollama, DeepSeek, and more
pip install reasongraph[graph-neo4j]          # Neo4j graph store (LPG)
pip install reasongraph[graph-falkordb]       # FalkorDB graph store (LPG)
pip install reasongraph[graph-apache-age]     # Apache AGE graph store (LPG)
pip install reasongraph[graph-amazon-neptune] # AWS Neptune graph store (LPG)
pip install reasongraph[tripletstore-oxigraph] # Embedded in-memory/on-disk RDF store
# RDF triple stores (Blazegraph, Apache Jena, Eclipse RDF4J) need no extra:
# reasongraph.triplet_store talks SPARQL over HTTP using the core `requests` dependency
pip install reasongraph[vectorstore-qdrant]   # Qdrant vector store
pip install reasongraph[vectorstore-pinecone] # Pinecone vector store
pip install reasongraph[db-snowflake]         # Snowflake
pip install reasongraph[db-databricks]        # Databricks (SDK + SQL connector)
pip install reasongraph[ingest-parquet]       # Parquet / PyArrow
pip install reasongraph[ingest-arrow]        # Apache Arrow, Feather, IPC
pip install reasongraph[viz]                  # HTML interactive visualization
pip install reasongraph[watch]                # Directory file watcher
pip install reasongraph[explorer]             # Knowledge Explorer dashboard
```

For production deployments, use Docker or Kubernetes rather than a local `pip install`. Set `REASONGRAPH_SECRET_KEY`, configure a persistent LPG graph store (Neo4j / FalkorDB / Apache AGE / AWS Neptune) and/or RDF triple store (Blazegraph / Apache Jena / Eclipse RDF4J), and point the vector store at a hosted backend (Qdrant / Pinecone). See [ARCHITECTURE.md](ARCHITECTURE.md) for the full deployment topology.

```bash
# From source
git clone https://github.com/savsuth/Reasongraph.git
cd reasongraph && pip install -e ".[dev]" && pytest tests/
```
...
MIT licensed. Requires Python 3.8 or newer.

| | |
| --- | --- |
| **Security** | Supported versions and vulnerability reporting: [SECURITY.md](SECURITY.md) |
| **Contributing** | [Contributing Guide](docs/contributing-guide.md) |
| **Support** | [SUPPORT.md](SUPPORT.md) |
| **Releases** | [CHANGELOG.md](CHANGELOG.md) · [Release notes](RELEASE_NOTES.md) |
