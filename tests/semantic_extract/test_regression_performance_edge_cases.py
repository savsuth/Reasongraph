...
from reasongraph.semantic_extract.config import resolve_max_workers
from reasongraph.semantic_extract.ner_extractor import Entity, NERExtractor
from reasongraph.semantic_extract.relation_extractor import RelationExtractor
from reasongraph.semantic_extract.triplet_extractor import TripletExtractor
from reasongraph.semantic_extract.semantic_network_extractor import SemanticNetworkExtractor
from reasongraph.semantic_extract.methods import filter_entities_for_text
from reasongraph.semantic_extract.schemas import RelationsResponse, RelationOut
...
def test_relation_llm_prompt_filter_does_not_break_mapping():
    ...
    with patch("reasongraph.semantic_extract.methods.create_provider", return_value=FakeLLM()):
        from reasongraph.semantic_extract.methods import extract_relations_llm
...
def test_triplet_extractor_reuses_sub_extractors():
    ...
    with patch("reasongraph.semantic_extract.ner_extractor.NERExtractor", ner_ctor), patch(
        "reasongraph.semantic_extract.relation_extractor.RelationExtractor", rel_ctor
    ), patch("reasongraph.semantic_extract.methods.get_triplet_method", return_value=lambda *args, **kwargs: []):
