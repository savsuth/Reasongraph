"""Tests for the SPARQL Explorer route (reasongraph/explorer/routes/sparql.py).
...
from reasongraph.context.context_graph import ContextGraph
from reasongraph.explorer.app import create_app
from reasongraph.explorer.session import GraphSession

try:
    from starlette.testclient import TestClient
except ImportError:
    pytest.skip(
        "starlette TestClient is required for explorer tests. Install reasongraph[explorer].",
        allow_module_level=True,
    )

import reasongraph.explorer.routes.sparql as sparql_mod
...
def test_construct_query_succeeds(client):
    ...
    assert any(
        row["subject"] == "http://reasongraph.local/entity/python"
...
def test_describe_query_succeeds(client):
    resp = _post(client, "DESCRIBE <http://reasongraph.local/entity/python>")
    assert resp.status_code == 200
    payload = resp.json()
    assert payload["error"] is None
    assert payload["columns"] == ["subject", "predicate", "object"]
    ...
    assert any(
        ...
        and row["object"] == "http://reasongraph.local/entity/language"
        for row in payload["rows"]
    )
...
def test_lowercase_query_keyword_is_accepted(client):
    ...
    resp = _post(client, "select ?s where { ?s a <http://reasongraph.local/entity/language> }")
    assert resp.status_code == 200
    payload = resp.json()
    assert payload["error"] is None
...
def test_select_with_no_results_returns_empty_rows(client):
    resp = _post(client, "SELECT ?s WHERE { ?s a <http://reasongraph.local/entity/nonexistent> }")
    assert resp.status_code == 200
    payload = resp.json()
    assert payload["error"] is None
    assert payload["rows"] == []
...
@pytest.mark.parametrize(
    "query",
    [
        "INSERT DATA { <http://reasongraph.local/entity/x> a <http://reasongraph.local/entity/hacked> }",
        "DELETE DATA { <http://reasongraph.local/entity/python> a <http://reasongraph.local/entity/language> }",
        "DELETE WHERE { ?s ?p ?o }",
        "DROP ALL",
...
@pytest.mark.parametrize(
    ...
    [
        "DROP ALL",
        "INSERT DATA { <http://reasongraph.local/entity/x> a <http://reasongraph.local/entity/y> }",
        "# comment\nDROP ALL",
        "",
    ],
)
...
def test_row_cap_truncates_results(client):
    with patch.object(sparql_mod, "_SPARQL_MAX_ROWS", 1):
        resp = _post(
            client,
            "SELECT ?s WHERE { ?s a <http://reasongraph.local/entity/language> }",
        )
    assert resp.status_code == 200
    payload = resp.json()
    assert payload["error"] is None
...
def test_node_properties_are_projected_as_literals_excluding_reserved_keys(client):
    resp = _post(
        client,
        "SELECT ?p ?v WHERE { <http://reasongraph.local/entity/python> ?p ?v }",
    )
    payload = resp.json()
...
def test_edges_are_projected_with_their_relationship_type(client):
    resp = _post(
        client,
        "SELECT ?o WHERE { <http://reasongraph.local/entity/python> "
        "<http://reasongraph.local/prop/used_in> ?o }",
    )
    payload = resp.json()
    assert payload["error"] is None
...
def test_concurrent_requests_all_complete_successfully(client):
    ...
    def _run():
        return _post(client, "SELECT ?s WHERE { ?s a <http://reasongraph.local/entity/language> }")
