...
from reasongraph.explorer.routes.ontology import OntologyEntry
...
def test_health_dedupes_node_edge_fetch(client):
    import reasongraph.explorer.routes.ontology as ont_mod
...
async def test_data_graph_turtle_resolves_onto_prefix_and_unknown_curies(client):
    ...
    from reasongraph.explorer.routes.ontology import _data_graph_turtle_for_uri
...
def test_ontology_namespace_helper_handles_all_uri_forms():
    from reasongraph.explorer.routes.ontology import _ontology_namespace
...
onto:PersonShape a sh:NodeShape ;
    sh:targetClass onto:Person .
"""
    with patch("reasongraph.explorer.routes.ontology._MAX_SHACL_TURTLE_BYTES", 20):
        response = client.post(
            "/api/ontology/shacl/validate",
            json={
                "uri": "http://example.org/onto-a",
...
onto:PersonShape a sh:NodeShape ;
    sh:targetClass onto:Person .
"""
    with patch("reasongraph.explorer.routes.ontology._MAX_SHACL_TRIPLES", 1):
        response = client.post(
            "/api/ontology/shacl/validate",
            json={
                "uri": "http://example.org/onto-a",
...
"""
    ...
    with patch("reasongraph.explorer.routes.ontology._MAX_SHACL_TIMEOUT_SECONDS", 0.05), \
         patch("reasongraph.ontology.OntologyEngine.validate_graph", side_effect=slow_validate):
        response = client.post(
            "/api/ontology/shacl/validate",
            json={
                "uri": "http://example.org/onto-a",
...
onto:PersonShape a sh:NodeShape ;
    sh:targetClass onto:Person .
"""
    with patch("reasongraph.explorer.routes.ontology._MAX_ANALYSIS_NODES", 0):
        response = client.post(
            "/api/ontology/shacl/validate",
            json={
                "uri": "http://example.org/onto-a",
...
def test_ontology_load_does_not_swallow_422_from_ingestor_success_path(client):
    ...
    from reasongraph.ingest.ontology_ingestor import OntologyData
    ...
    with patch(
        ...
        "reasongraph.explorer.session.GraphSession.add_nodes_and_edges",
...
def test_refresh_ontology_success_adds_nodes_and_edges(client):
    ...
    with patch(
        "reasongraph.explorer.routes.ontology._fetch_url_sync",
