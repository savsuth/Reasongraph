...
# Add parent directory to sys.path to import reasongraph
PARENT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
sys.path.insert(0, PARENT_DIR)

# Import modules
import reasongraph.utils.exceptions
import reasongraph.semantic_extract.methods
import reasongraph.semantic_extract.triplet_extractor

from reasongraph.semantic_extract.methods import extract_entities_llm, extract_relations_llm, extract_triplets_llm
from reasongraph.semantic_extract.triplet_extractor import TripletExtractor, Triplet
...
print(f"DEBUG: reasongraph.semantic_extract.methods file: {reasongraph.semantic_extract.methods.__file__}")
print(f"DEBUG: reasongraph.semantic_extract.triplet_extractor file: {reasongraph.semantic_extract.triplet_extractor.__file__}")

class TestLLMExtractionFixes(unittest.TestCase):

    @patch('reasongraph.semantic_extract.methods.create_provider')
    ...
    def test_chunking_detection(self, mock_create):
        ...
        with patch('reasongraph.semantic_extract.methods._extract_entities_chunked') as mock_chunked:
