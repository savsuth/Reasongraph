...
from reasongraph.context.agent_memory import AgentMemory
...
def test_markdown_string_path_inspection_errors_are_actionable():
    ...
    with patch(
        "reasongraph.context.agent_memory.Path.exists",
...
def test_markdown_import_rejects_non_regular_file(tmp_path):
    ...
    memory = AgentMemory()
    with patch("reasongraph.context.agent_memory.os.fstat", return_value=fake_stat):
