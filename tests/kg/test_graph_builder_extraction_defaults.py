...
class TestGraphBuilderExtractionDefaults(unittest.TestCase):
    ...
    def setUp(self):
        self.ner_patcher = patch(
            "reasongraph.semantic_extract.ner_extractor.NERExtractor"
        )
        self.rel_patcher = patch(
            "reasongraph.semantic_extract.relation_extractor.RelationExtractor"
        )
        self.trip_patcher = patch(
            "reasongraph.semantic_extract.triplet_extractor.TripletExtractor"
        )
...
class TestGraphBuilderDefaultsRunOffline(unittest.TestCase):
    ...
    def test_default_build_needs_no_provider(self):
        ...
        with patch("reasongraph.semantic_extract.providers.create_provider") as provider:
