...
from reasongraph.kg.graph_builder import GraphBuilder
from reasongraph.kg.graph_analyzer import GraphAnalyzer
from reasongraph.kg.temporal_model import TemporalBound, deserialize_relationship_temporal_fields
from reasongraph.kg.temporal_query import TemporalConsistencyReport, validate_temporal_consistency
from reasongraph.utils.exceptions import TemporalValidationError

class TestGraphBuilder(unittest.TestCase):
    def setUp(self):
        # Patch where it is defined since it is imported inside __init__
        self.mock_tracker_patcher = patch("reasongraph.utils.progress_tracker.get_progress_tracker")
        self.mock_get_tracker = self.mock_tracker_patcher.start()
        self.mock_tracker = MagicMock()
        self.mock_get_tracker.return_value = self.mock_tracker

        self.mock_resolver_patcher = patch("reasongraph.kg.entity_resolver.EntityResolver")
        self.mock_resolver_cls = self.mock_resolver_patcher.start()
        
        self.mock_conflict_patcher = patch("reasongraph.conflicts.conflict_detector.ConflictDetector")
...
class TestGraphAnalyzer(unittest.TestCase):
    def setUp(self):
        self.mock_tracker_patcher = patch("reasongraph.kg.graph_analyzer.get_progress_tracker")
        self.mock_get_tracker = self.mock_tracker_patcher.start()
        self.mock_get_tracker.return_value = MagicMock()

        self.mock_centrality_patcher = patch("reasongraph.kg.graph_analyzer.CentralityCalculator")
        self.mock_centrality_cls = self.mock_centrality_patcher.start()
        self.mock_centrality = self.mock_centrality_cls.return_value

        self.mock_community_patcher = patch("reasongraph.kg.graph_analyzer.CommunityDetector")
        self.mock_community_cls = self.mock_community_patcher.start()
        self.mock_community = self.mock_community_cls.return_value

        self.mock_connectivity_patcher = patch("reasongraph.kg.graph_analyzer.ConnectivityAnalyzer")
...
class TestTemporalGraphQuery(unittest.TestCase):
    def setUp(self):
        ...
        from reasongraph.kg.temporal_query import TemporalGraphQuery
    ...
    def test_query_temporal_pattern_sequence_supports_gap_tolerance(self):
        query_engine = __import__("reasongraph.kg.temporal_query", fromlist=["TemporalGraphQuery"]).TemporalGraphQuery(
