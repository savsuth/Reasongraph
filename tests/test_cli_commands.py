...
import reasongraph.cli as cli_module
...
class TestGlobalFlags:
    ...
    def test_default_log_file_failure_falls_back_without_polluting_json(
        ...
        def fake_setup_logging(*, config=None, **_kwargs):
            ...
            if len(calls) == 1:
                raise PermissionError("readonly reasongraph.log")
        ...
        assert "readonly reasongraph.log" not in result.output
...
class TestKgSubcommands:
    ...
    def test_kg_query_json_with_mock(self, runner, monkeypatch):
        ...
        monkeypatch.setitem(__import__("sys").modules, "reasongraph.graph_store", fake_gs)
    ...
    def test_kg_stats_json_with_mock(self, runner, monkeypatch):
        ...
        monkeypatch.setitem(__import__("sys").modules, "reasongraph.kg", fake_kg)
...
class TestIngest:
    ...
    def test_runtime_path_passes_source_positionally(self, runner, monkeypatch):
        ...
        monkeypatch.setattr("reasongraph.ingest.methods.ingest_file", fake_ingest_file)
    ...
    def test_import_error_is_clean(self, runner, monkeypatch):
        ...
        with patch("builtins.__import__", side_effect=lambda n, *a, **k: (
            ...
            if n.startswith("reasongraph.ingest") else original_import(n, *a, **k)
        )):
...
class TestParse:
    ...
    def test_parse_real_file(self, runner):
        with runner.isolated_filesystem():
            ...
            with patch("builtins.__import__", side_effect=lambda n, *a, **k: (
                (_ for _ in ()).throw(ImportError(n))
                if n.startswith("reasongraph.parse") else __import__(n, *a, **k)
            )):
...
class TestSplit:
    ...
    def test_split_with_import_error(self, runner):
        with runner.isolated_filesystem():
            ...
            with patch("builtins.__import__", side_effect=lambda n, *a, **k: (
                (_ for _ in ()).throw(ImportError(n))
                if n.startswith("reasongraph.split") else __import__(n, *a, **k)
            )):
    ...
    def test_split_writes_output_file(self, runner, monkeypatch):
        ...
        monkeypatch.setitem(
            __import__("sys").modules, "reasongraph.split", fake_split
        )
        with runner.isolated_filesystem():
            with open("doc.txt", "w") as f:
                f.write("line1\nline2")
...
class TestNormalize:
    ...
    def test_normalize_text_inline(self, runner, monkeypatch):
        ...
        monkeypatch.setitem(
            __import__("sys").modules, "reasongraph.normalize", fake_norm
        )
...
class TestExtract:
    ...
    def test_dry_run_not_needed_extract_is_read_only(self, runner, monkeypatch):
        ...
        monkeypatch.setitem(
            __import__("sys").modules, "reasongraph.semantic_extract", fake_ext
        )
        result = runner.invoke(
    ...
    def test_import_error_is_clean(self, runner):
        with patch("builtins.__import__", side_effect=lambda n, *a, **k: (
            (_ for _ in ()).throw(ImportError(n))
            if n.startswith("reasongraph.semantic_extract") else __import__(n, *a, **k)
        )):
...
class TestEmbed:
    ...
    def test_index_loads_vectors_from_json(self, runner, monkeypatch, tmp_path):
        ...
        monkeypatch.setitem(__import__("sys").modules, "reasongraph.vector_store", fake_vs)
...
class TestDeduplicate:
    ...
    def test_detect_runtime_path(self, runner, monkeypatch):
        ...
        monkeypatch.setattr("reasongraph.graph_store.methods._get_store", lambda: FakeStore())
        monkeypatch.setattr("reasongraph.graph_store.methods.get_nodes", lambda **kwargs: entities)
    ...
    def test_merge_runtime_path(self, runner, monkeypatch):
        ...
        monkeypatch.setattr(
            "reasongraph.deduplication.entity_merger.EntityMerger.merge_duplicates",
            fake_merge,
        )
...
class TestDecision:
    ...
    def test_record_import_error_is_clean(self, runner):
        with patch("builtins.__import__", side_effect=lambda n, *a, **k: (
            (_ for _ in ()).throw(ImportError(n))
            if "reasongraph.context" in n else __import__(n, *a, **k)
        )):
    ...
    def test_list_json(self, runner, monkeypatch):
        ...
        monkeypatch.setitem(__import__("sys").modules,
                            "reasongraph.context.decision_query", fake_decision_query)
        monkeypatch.setitem(__import__("sys").modules,
                            "reasongraph.graph_store", fake_graph_store)
...
class TestTemporal:
    ...
    def test_history_import_error_is_clean(self, runner):
        with patch("builtins.__import__", side_effect=lambda n, *a, **k: (
            (_ for _ in ()).throw(ImportError(n))
            if n.startswith("reasongraph.kg") else __import__(n, *a, **k)
        )):
...
class TestProvenance:
    ...
    def test_audit_writes_output(self, runner, monkeypatch):
        ...
        monkeypatch.setitem(__import__("sys").modules, "reasongraph.provenance", fake_prov)
        with runner.isolated_filesystem():
...
class TestValidate:
    ...
    def test_conflicts_json(self, runner, monkeypatch):
        ...
        monkeypatch.setitem(__import__("sys").modules, "reasongraph.conflicts", fake_conf)
...
class TestExport:
    ...
    def test_real_export_runtime_path(self, runner, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "reasongraph.graph_store.methods._get_store",
            lambda: FakeGraphStore(),
        )
        monkeypatch.setattr(
            "reasongraph.graph_store.get_nodes",
            lambda **kwargs: [
                {
                    "id": "n1",
                    "type": "Person",
        ...
        monkeypatch.setattr(
            "reasongraph.graph_store.methods.get_nodes",
            lambda **kwargs: [
                {
                    "id": "n1",
                    "type": "Person",
        ...
        monkeypatch.setattr(
            "reasongraph.graph_store.get_relationships",
            lambda **kwargs: [
                {
                    "id": "r1",
                    "source": "n1",
        ...
        monkeypatch.setattr(
            "reasongraph.graph_store.methods.get_relationships",
            lambda **kwargs: [
                {
                    "id": "r1",
                    "source": "n1",
    ...
    def test_import_error_is_clean(self, runner):
        ...
        with patch("builtins.__import__", side_effect=lambda n, *a, **k: (
            (_ for _ in ()).throw(ImportError(n))
            if "reasongraph.export" in n else original_import(n, *a, **k)
        )):
...
class TestPipeline:
    ...
    def test_init_creates_file(self, runner, monkeypatch):
        ...
        monkeypatch.setitem(__import__("sys").modules, "reasongraph.pipeline", fake_pl)
        with runner.isolated_filesystem():
...
class TestExplorer:
    ...
    def test_start_forwards_api_url_to_child_environment(self, runner, monkeypatch):
        ...
        _ok(result)
        assert mock_popen.call_args.kwargs["env"]["REASONGRAPH_API_URL"] == (
            "http://localhost:9000"
        )
