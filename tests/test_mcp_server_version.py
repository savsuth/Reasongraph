...
package version rather than a hardcoded stale literal:
  ...
  2. ``reasongraph://schema/info`` → ``version``

The authoritative source of truth is ``reasongraph.__version__``, which is
...
import reasongraph
from reasongraph import mcp_server

_EXPECTED = reasongraph.__version__
...
class TestMCPServerVersion(unittest.TestCase):
    ...
    # ------------------------------------------------------------------ #
    # reasongraph://schema/info → version
    # ------------------------------------------------------------------ #

    def test_schema_info_resource_version_matches_package(self):
        """The reasongraph://schema/info resource must report the authoritative package version."""
        resource = mcp_server._read_resource("reasongraph://schema/info")
    ...
    def test_both_version_surfaces_are_identical(self):
        ...
        schema_resource = mcp_server._read_resource("reasongraph://schema/info")
        self.assertEqual(
