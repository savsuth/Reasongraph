...
# even if other tests already loaded reasongraph (which would have cached the module
# with its original psycopg2 binding, making the sys.modules patch above a no-op).
for _key in list(sys.modules.keys()):
    if "reasongraph" in _key and "age_store" in _key:
        del sys.modules[_key]

from reasongraph.graph_store.age_store import (
    ApacheAgeStore,
    _edge_to_rel_dict,
    _parse_agtype,
...
class TestApacheAgeStore(unittest.TestCase):
    ...
    def test_create_node_with_reasongraph_id(self):
        """reasongraph_id in properties should be preserved."""
        vertex_str = _make_vertex_agtype(
            102, "Entity", {"reasongraph_id": "abc-123", "value": "test"}
        )
        self.mock_cursor.fetchall.return_value = [(vertex_str,)]

        node = self.store.create_node(
            labels=["Entity"],
            properties={"reasongraph_id": "abc-123", "value": "test"},
        )

        self.assertEqual(node["id"], 102)  # AGE internal ID
        self.assertEqual(node["properties"]["reasongraph_id"], "abc-123")
...
class TestGraphStoreFacadeAge(unittest.TestCase):
    ...
    @patch("reasongraph.graph_store.age_store.ApacheAgeStore", autospec=True)
    def test_age_backend_initialisation(self, MockAgeStore):
        """GraphStore should instantiate ApacheAgeStore for 'age' backend."""
        from reasongraph.graph_store.graph_store import GraphStore
