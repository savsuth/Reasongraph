...
from reasongraph.utils.exceptions import ProcessingError  # noqa: E402
from reasongraph.visualization.kg_visualizer import KGVisualizer  # noqa: E402
...
def _make_viz():
    ...
    patches = [
        patch("reasongraph.visualization.kg_visualizer.get_logger", return_value=mock_logger),
        patch("reasongraph.visualization.kg_visualizer.get_progress_tracker", return_value=mock_tracker),
...
class TestVisualizeNetworkAcceptsKGObject(unittest.TestCase):
    ...
    def _run_visualize_network(self, graph_arg):
        ...
        with (
            patch(
                "reasongraph.visualization.kg_visualizer.ColorPalette.get_entity_type_colors",
                return_value={"Person": "#ff0000"},
            ),
            patch(
                "reasongraph.visualization.kg_visualizer.ColorPalette.get_colors",
                return_value=["#ff0000"],
            ),
        ):
...
class TestAllVisualizeMethodsAcceptKGObject(unittest.TestCase):
    ...
    def test_visualize_communities_accepts_kg_object(self):
        ...
        with patch(
            "reasongraph.visualization.kg_visualizer.ColorPalette.get_community_colors",
...
class TestFormalKnowledgeGraphType(unittest.TestCase):
    ...
    The formal ``reasongraph.kg.KnowledgeGraph`` dataclass must be accepted by
    ...
    def setUpClass(cls):
        try:
            from reasongraph.kg.knowledge_graph import KnowledgeGraph
    ...
    def _make_kg(self):
        if self.KnowledgeGraph is None:
            self.skipTest("reasongraph.kg.KnowledgeGraph not available")
    ...
    def test_knowledge_graph_importable_from_kg_module(self):
        if self.KnowledgeGraph is None:
            self.skipTest("reasongraph.kg.KnowledgeGraph not available")
        try:
            import reasongraph.kg as _kg_module
            _ = _kg_module.KnowledgeGraph
        except (ImportError, AttributeError) as exc:
            self.fail(f"KnowledgeGraph not exported from reasongraph.kg: {exc}")
