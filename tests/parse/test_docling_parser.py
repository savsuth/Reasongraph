...
from reasongraph.parse.docling_parser import DoclingParser, DoclingMetadata

class TestDoclingParser(unittest.TestCase):
    def setUp(self):
        # Patch DOCLING_AVAILABLE to True for testing logic
        self.available_patcher = patch('reasongraph.parse.docling_parser.DOCLING_AVAILABLE', True)
        self.available_patcher.start()
        
        # Mock the DocumentConverter
        self.mock_converter_cls = patch('reasongraph.parse.docling_parser.DocumentConverter').start()
