"""
Comprehensive Real-World Test Suite for Reasongraph 0.3.0-alpha and 0.3.0-beta
...
from reasongraph.context.decision_models import (
    ApprovalChain,
    PolicyException,
    Precedent,
...
class TestReasonerPatternMatching:
    ...
    def test_multi_word_value_inference(self):
        ...
        from reasongraph.reasoning.reasoner import Reasoner
        reasoner = Reasoner()
...
class TestPipelineRetryAndFailure:
    ...
    def test_failure_handler_linear_backoff(self):
        from reasongraph.pipeline.failure_handler import FailureHandler, RetryPolicy, RecoveryAction, RetryStrategy
...
# ═════════════════════════════════════════════════════════════════════════════
# 14. ProvenanceTracker – now exported from reasongraph.kg (0.3.0-beta fix)
# ═════════════════════════════════════════════════════════════════════════════

class TestProvenanceTrackerExport:
    """ProvenanceTracker must be importable from reasongraph.kg after the fix."""
...
class TestSemanticExtractRelationFix:
    ...
    def test_llm_relation_not_duplicated(self):
        ...
        from reasongraph.semantic_extract.schemas import RelationOut, RelationsResponse
        ...
        with patch("reasongraph.semantic_extract.methods.create_provider", return_value=_FakeLLM()):
