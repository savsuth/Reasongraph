...
import reasongraph.utils.progress_tracker as progress_module


@pytest.fixture(autouse=True)
def reset_progress_singletons(monkeypatch):
    monkeypatch.delenv("REASONGRAPH_DISABLE_PROGRESS", raising=False)
...
def test_disable_progress_env_prevents_reenable(monkeypatch):
    monkeypatch.setenv("REASONGRAPH_DISABLE_PROGRESS", "1")
    progress_module.ProgressTracker._instance = None
    progress_module._global_tracker = None
...
def test_build_knowledge_base_subprocess_does_not_deadlock():
    ...
    sample.write_text("Reasongraph builds small local knowledge graphs.", encoding="utf-8")

    code = (
        "from reasongraph.core import Reasongraph; "
        f"result = Reasongraph().build_knowledge_base([{str(sample)!r}], embeddings=False, graph=False); "
