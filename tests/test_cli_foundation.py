...
def test_root_help_shows_expected_groups(runner):
    ...
    assert "reasongraph  v" in result.output
...
def test_info_command_shows_framework_components(runner):
    ...
    assert result.exit_code == 0
    assert "Reasongraph Framework" in result.output
...
class _MockFramework:
    """Lightweight stand-in for Reasongraph used to test _run_build result paths."""
...
def test_log_directory_probe_does_not_delete_active_log_file(monkeypatch):
    """doctor probes with a temp file instead of touching reasongraph.log."""
    work_dir = _runtime_dir("doctor-log-probe")
    monkeypatch.chdir(work_dir)
    log_path = work_dir / "reasongraph.log"
...
def test_doctor_json_log_directory_check_is_not_false_failure(runner, monkeypatch):
    ...
    monkeypatch.chdir(work_dir)
    (work_dir / "reasongraph.log").write_text("existing log", encoding="utf-8")
...
def test_require_ctx_returns_ctx_unchanged():
    ...
    from reasongraph.core.config_manager import Config, ConfigManager
