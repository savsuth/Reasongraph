...
from reasongraph.core.config_manager import ConfigManager, Config, ConfigurationError
from reasongraph.core.lifecycle import LifecycleManager, SystemState, HealthStatus
from reasongraph.core.plugin_registry import PluginRegistry, PluginInfo
from reasongraph.core.registry import method_registry, MethodRegistry
from reasongraph.core.orchestrator import Reasongraph
from reasongraph.core import methods
...
class TestPluginRegistry(unittest.TestCase):
    def setUp(self):
        self.patcher = patch("reasongraph.core.plugin_registry.get_progress_tracker")
...
class TestReasongraphOrchestrator(unittest.TestCase):
    def setUp(self):
        self.patcher = patch("reasongraph.core.orchestrator.get_progress_tracker")
        self.mock_get_tracker = self.patcher.start()
        self.mock_get_tracker.return_value = MagicMock()
    ...
    @patch("reasongraph.core.orchestrator.LifecycleManager.startup")
    def test_initialize(self, mock_startup):
        self.reasongraph.initialize()
        self.assertTrue(self.reasongraph._initialized)
        mock_startup.assert_called_once()

    @patch("reasongraph.core.orchestrator.Reasongraph._create_pipeline")
    @patch("reasongraph.core.orchestrator.Reasongraph._validate_sources")
...
class TestCoreMethods(unittest.TestCase):
    @patch("reasongraph.core.methods.Reasongraph")
    def test_build_knowledge_base_wrapper(self, MockReasongraph):
        mock_instance = MockReasongraph.return_value
        mock_instance.build_knowledge_base.return_value = {"status": "ok"}
        
        res = methods.build_knowledge_base(sources=["file.txt"])
        
        MockReasongraph.assert_called_once()
