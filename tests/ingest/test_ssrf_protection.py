...
from reasongraph.ingest.api_ingestor import RESTIngestor
from reasongraph.ingest.ssrf import (
    parse_bool,
    request_with_ssrf_guard,
    validate_url_for_request,
)
from reasongraph.ingest.web_ingestor import SitemapCrawler, WebIngestor
...
class TestValidateUrlForRequest:
    def test_accepts_https(self):
        with patch(
            "reasongraph.ingest.ssrf.socket.getaddrinfo",
    ...
    def test_dns_timeout_raises(self):
        ...
        with patch(
            "reasongraph.ingest.ssrf.concurrent.futures.Future.result",
    ...
    def test_hung_getaddrinfo_returns_within_timeout_bound(self):
        ...
        import reasongraph.ingest.ssrf as ssrf
...
class TestWebIngestorSSRF:
    ...
    def test_file_scheme_rejected_by_reasongraph(self):
        ingestor = WebIngestor(respect_robots=False, delay=0)
        with pytest.raises(ValidationError, match="not permitted"):
