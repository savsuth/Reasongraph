...
from reasongraph.ingest.api_ingestor import RESTIngestor, APIData
from reasongraph.ingest.duckdb_ingestor import DuckDBIngestor, DuckDBData
from reasongraph.ingest.elastic_ingestor import ElasticIngestor, ElasticData
from reasongraph.ingest.mcp_ingestor import MCPIngestor, MCPData
from reasongraph.ingest.mcp_client import MCPClient, MCPResource, MCPTool
from reasongraph.ingest.gdrive_ingestor import GDriveIngestor, GDriveData
from reasongraph.ingest.huggingface_ingestor import HuggingFaceIngestor, HFData
from reasongraph.ingest.mongo_ingestor import MongoIngestor, MongoData, MongoConnector
from reasongraph.ingest.pandas_ingestor import PandasIngestor, PandasData
from reasongraph.ingest.repo_ingestor import RepoIngestor, CodeFile
from reasongraph.ingest.stream_ingestor import StreamIngestor
...
class TestDuckDBIngestor:
    def test_init_raises_if_no_duckdb(self):
        ...
        with patch("reasongraph.ingest.duckdb_ingestor.duckdb", None):
    ...
    def test_ingest_csv(self):
        ...
        try:
            ...
            with patch("reasongraph.ingest.duckdb_ingestor.duckdb", mock_duckdb):
...
class TestElasticIngestor:
    def test_init_raises_if_no_elastic(self):
        with patch("reasongraph.ingest.elastic_ingestor.Elasticsearch", None):
            with pytest.raises(ImportError):
                ElasticIngestor()
    ...
    def test_ingest_index(self):
        ...
        with patch("reasongraph.ingest.elastic_ingestor.Elasticsearch", mock_es_class), \
             patch("reasongraph.ingest.elastic_ingestor.scan", mock_scan):
...
class TestMCPIngestor:
    def test_connect_and_ingest(self):
        ...
        with patch("reasongraph.ingest.mcp_ingestor.MCPClient") as MockClient, \
             patch("reasongraph.ingest.mcp_ingestor.get_progress_tracker") as mock_get_tracker:
...
class TestGDriveIngestor:
    def test_init_raises_if_no_google_libs(self):
        with patch("reasongraph.ingest.gdrive_ingestor.build", None):
            with pytest.raises(ImportError):
                GDriveIngestor()
    ...
    def test_ingest_folder(self):
        ...
        # Mock downloader
        with patch("reasongraph.ingest.gdrive_ingestor.MediaIoBaseDownload") as MockDownloader, \
             patch("reasongraph.ingest.gdrive_ingestor.build") as mock_build, \
             patch("reasongraph.ingest.gdrive_ingestor.InstalledAppFlow"), \
             patch("reasongraph.ingest.gdrive_ingestor.Credentials"):
...
class TestHuggingFaceIngestor:
    def test_init_raises_if_no_datasets(self):
        with patch("reasongraph.ingest.huggingface_ingestor.load_dataset", None):
            with pytest.raises(ImportError):
                HuggingFaceIngestor()

    def test_ingest_dataset(self):
        with patch("reasongraph.ingest.huggingface_ingestor.load_dataset") as mock_load:
            # Mock dataset
            mock_data = [
                {"col1": "val1", "col2": 1},
                {"col1": "val2", "col2": 2}
...
class TestMongoIngestor:
    def test_init_raises_if_no_pymongo(self):
        with patch("reasongraph.ingest.mongo_ingestor.MongoClient", None):
            with pytest.raises(ImportError):
                MongoIngestor()

    def test_ingest_collection(self):
        with patch("reasongraph.ingest.mongo_ingestor.MongoClient") as MockClient:
            mock_client = MockClient.return_value
            mock_db = MagicMock()
            mock_coll = MagicMock()
...
class TestRepoIngestor:
    def test_ingest_repository(self):
        ...
        try:
            ...
            with patch("reasongraph.ingest.repo_ingestor.git.Repo") as MockRepo, \
                 patch("reasongraph.ingest.repo_ingestor.tempfile.mkdtemp") as mock_mkdtemp, \
                 patch("reasongraph.ingest.repo_ingestor.shutil.rmtree"), \
                 patch("reasongraph.ingest.repo_ingestor.get_progress_tracker") as mock_get_tracker:
...
class TestStreamIngestor:
    def test_ingest_kafka(self):
        with patch("reasongraph.ingest.stream_ingestor.KafkaProcessor") as MockProcessor:
