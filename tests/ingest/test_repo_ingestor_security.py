...
from reasongraph.ingest import repo_ingestor as repo_ingestor_mod
from reasongraph.ingest.repo_ingestor import (
...
class TestRepoUrlValidation:
    def test_accepts_https_github_url(self):
        with patch(
            "reasongraph.ingest.repo_ingestor.socket.getaddrinfo",
    ...
    def test_malformed_url_surfaces_as_validation_error_from_ingest(self):
        with patch("reasongraph.ingest.repo_ingestor.git.Repo") as MockRepo, patch(
            "reasongraph.ingest.repo_ingestor.get_progress_tracker"
        ) as mock_get_tracker:
...
class TestIngestRepositoryGuards:
    ...
    def test_safe_options_forwarded_to_clone_from(self):
        with patch("reasongraph.ingest.repo_ingestor.git.Repo") as MockRepo, patch(
            "reasongraph.ingest.repo_ingestor.tempfile.mkdtemp",
            return_value="/tmp/fake-repo",
        ), patch("reasongraph.ingest.repo_ingestor.shutil.rmtree"), patch(
            "reasongraph.ingest.repo_ingestor.get_progress_tracker"
        ) as mock_get_tracker, patch(
            "reasongraph.ingest.repo_ingestor.socket.getaddrinfo",
            return_value=_fake_addrinfo("140.82.112.3"),
