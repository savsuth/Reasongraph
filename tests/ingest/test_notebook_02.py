...
from reasongraph.ingest import (
    ingest, 
...
class TestNotebook02DataIngestion:
    ...
    def test_01_unified_ingestion(self):
        ...
        with open(sample_file, 'w') as f:
            f.write("Reasongraph Unified Ingestion Example")
        ...
        with patch("reasongraph.ingest.web_ingestor.WebIngestor.ingest_url") as mock_ingest:
    ...
    def test_03_web_ingestion(self):
        ...
        with patch("requests.Session") as MockSession:
            ...
            with patch("reasongraph.ingest.web_ingestor.RobotsChecker.can_fetch", return_value=True):
    ...
    def test_05_stream_ingestion(self):
        ...
        # Mock Kafka/RabbitMQ
        with patch("reasongraph.ingest.stream_ingestor.StreamIngestor.ingest_kafka") as mock_kafka:
            mock_kafka.return_value = MagicMock()
            stream_ingestor.ingest_kafka("my-topic", bootstrap_servers=["localhost:9092"])
            
        with patch("reasongraph.ingest.stream_ingestor.StreamIngestor.ingest_rabbitmq") as mock_rabbit:
    ...
    def test_06_repo_ingestion(self):
        ...
        with patch.object(repo_ingestor, 'ingest_repository') as mock_ingest:
            mock_ingest.return_value = {'name': 'reasongraph'}
            repo_data = repo_ingestor.ingest_repository("https://github.com/savsuth/Reasongraph.git")
            assert repo_data['name'] == 'reasongraph'
