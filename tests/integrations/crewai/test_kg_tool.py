"""
Tests for ReasongraphKGTool — knowledge graph CrewAI tool.
...
from integrations.crewai import ReasongraphKGTool as ImportedReasongraphKGTool
from integrations.crewai.kg_tool import (
    CREWAI_AVAILABLE,
    CREWAI_IMPORT_ERROR,
    ReasongraphKGTool,
    ReasongraphKGToolInput,
)
...
class _DataclassNER:
    """Returns Reasongraph's real ``Entity`` dataclass shape (text/label, no name)."""

    def extract_entities(self, text):
        from reasongraph.semantic_extract.types import Entity
...
class _DataclassRelExtractor:
    """Returns Reasongraph's real ``Relation`` dataclass shape (subject/object)."""

    def __init__(self):
        self.received_entities = None

    def extract_relations(self, text, entities=None):
        from reasongraph.semantic_extract.types import Entity, Relation
...
class TestReasongraphKGToolInit(unittest.TestCase):
    ...
    def test_is_base_tool_subclass(self):
        ...
        self.assertTrue(issubclass(ReasongraphKGTool, BaseTool))

    def test_exposed_from_package_init(self):
        self.assertIs(ImportedReasongraphKGTool, ReasongraphKGTool)

    def test_creates_with_explicit_graph(self):
        graph = ContextGraph()
        tool = ReasongraphKGTool(graph=graph)
        self.assertIs(tool.graph, graph)

    def test_creates_fresh_graph_when_none(self):
        tool = ReasongraphKGTool(
    ...
    def test_default_metadata(self):
        ...
        self.assertEqual(tool.name, "reasongraph_knowledge_graph")
        self.assertTrue(tool.description)
        self.assertEqual(tool.args_schema, ReasongraphKGToolInput)

    def test_input_schema_validates(self):
        inp = ReasongraphKGToolInput(action="query_graph", query="privacy", hops=2)
        self.assertEqual(inp.hops, 2)
        with self.assertRaises(Exception):
            ReasongraphKGToolInput(action="bogus")
...
class TestReasongraphKGToolSerialization(unittest.TestCase):
    ...
    def test_model_dump_json_excludes_shared_state(self):
        ...
        self.assertEqual(dumped["name"], "reasongraph_knowledge_graph")

    def test_model_validate_restores_defaults(self):
        restored = ReasongraphKGTool.model_validate(self.tool.model_dump(mode="json"))
        self.assertIsInstance(restored.graph, ContextGraph)
        self.assertIs(restored.args_schema, ReasongraphKGToolInput)
        self.assertEqual(restored.name, "reasongraph_knowledge_graph")
    ...
    def test_restore_flags_lost_live_state(self):
        ...
        restored = ReasongraphKGTool.model_validate(dumped)
        self.assertTrue(restored.reconstructed_state)
        self.assertFalse(ReasongraphKGTool().reconstructed_state)


class TestReasongraphKGToolActions(unittest.TestCase):
...
class TestReasongraphKGToolDataclassShapes(unittest.TestCase):
    """Real Reasongraph ``Entity``/``Relation`` dataclasses (text/label,
...
class TestReasongraphKGToolCrewAIEntrypoints(unittest.TestCase):
