"""
Tests for ReasongraphKnowledgeSource — CrewAI knowledge source backed by a
Reasongraph ContextGraph.
...
class TestReasongraphKnowledgeSourceInit(unittest.TestCase):
    ...
    def test_is_base_knowledge_source_subclass(self):
        ...
        self.assertTrue(issubclass(ReasongraphKnowledgeSource, BaseKnowledgeSource))

    def test_creates_with_explicit_graph(self):
        graph = _build_graph()
        src = ReasongraphKnowledgeSource(graph=graph)
        self.assertIs(src.graph, graph)

    def test_creates_fresh_graph_when_none(self):
        src = ReasongraphKnowledgeSource()
        self.assertIsNotNone(src.graph)
        self.assertIsInstance(src.graph, ContextGraph)

    def test_default_metadata(self):
        src = ReasongraphKnowledgeSource(graph=_build_graph())
        self.assertEqual(src.name, "reasongraph_knowledge_graph")
...
class TestLoadContent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.src = ReasongraphKnowledgeSource(graph=self.graph)
    ...
    def test_empty_graph_returns_empty(self):
        src = ReasongraphKnowledgeSource(graph=ContextGraph())
...
class TestAdd(unittest.TestCase):
    ...
    def setUp(self):
        self.graph = _build_graph()
        self.src = ReasongraphKnowledgeSource(
    ...
    def test_add_wired_storage_failure_logs_error_not_debug(self):
        ...
        with self.assertLogs(
            f"reasongraph.{ReasongraphKnowledgeSource.__module__}", level="ERROR"
        ) as caught:
            self.src.add()
        joined = "\n".join(caught.output)
    ...
    def test_content_summary(self):
        ...
        self.assertEqual(summary["name"], "reasongraph_knowledge_graph")
        self.assertGreater(summary["source_count"], 0)
        self.assertTrue(summary["crewai_available"])


class TestReasongraphKnowledgeSourceSerialization(unittest.TestCase):
    ...
    def test_model_validate_restores_graph(self):
        src = ReasongraphKnowledgeSource(graph=_build_graph())
        restored = ReasongraphKnowledgeSource.model_validate(src.model_dump(mode="json"))
    ...
    def test_restore_flags_lost_live_state(self):
        ...
        restored = ReasongraphKnowledgeSource.model_validate(dumped)
        self.assertTrue(restored.reconstructed_state)
        self.assertFalse(ReasongraphKnowledgeSource().reconstructed_state)
        self.assertIsInstance(ReasongraphKnowledgeSource().graph, ContextGraph)
