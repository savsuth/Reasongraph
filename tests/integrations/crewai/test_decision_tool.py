"""
Tests for ReasongraphDecisionTool — decision intelligence CrewAI tool.
...
class TestReasongraphDecisionToolInit(unittest.TestCase):
    ...
    def test_is_base_tool_subclass(self):
        ...
        self.assertTrue(issubclass(ReasongraphDecisionTool, BaseTool))

    def test_creates_with_explicit_context(self):
        ctx = _make_context()
        tool = ReasongraphDecisionTool(context=ctx)
        self.assertIs(tool.context, ctx)

    def test_creates_context_when_none(self):
        tool = ReasongraphDecisionTool()
        self.assertIsNotNone(tool.context)

    def test_default_metadata(self):
        tool = ReasongraphDecisionTool(context=_make_context())
        self.assertEqual(tool.name, "reasongraph_decision")
        self.assertTrue(tool.description)
        self.assertEqual(tool.args_schema, ReasongraphDecisionToolInput)

    def test_input_schema_validates(self):
        inp = ReasongraphDecisionToolInput(action="record_decision", confidence=0.5)
        self.assertEqual(inp.confidence, 0.5)
        with self.assertRaises(Exception):
            ReasongraphDecisionToolInput(action="bogus")
...
class TestReasongraphDecisionToolSerialization(unittest.TestCase):
    ...
    def test_model_validate_restores_defaults(self):
        tool = ReasongraphDecisionTool(context=_make_context())
        restored = ReasongraphDecisionTool.model_validate(tool.model_dump(mode="json"))
    ...
    def test_restore_flags_lost_live_state(self):
        ...
        restored = ReasongraphDecisionTool.model_validate(dumped)
        self.assertTrue(restored.reconstructed_state)
        self.assertFalse(ReasongraphDecisionTool().reconstructed_state)
...
class TestRecordDecision(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tool = ReasongraphDecisionTool(context=self.ctx)
...
class TestRealAutoCreatedContext(unittest.TestCase):
    ...
    def setUp(self):
        self.tool = ReasongraphDecisionTool()
