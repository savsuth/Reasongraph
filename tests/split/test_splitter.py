...
from reasongraph.split.splitter import TextSplitter
from reasongraph.split.semantic_chunker import SemanticChunker, Chunk

class TestSplitter(unittest.TestCase):

    def setUp(self):
        self.mock_logger = MagicMock()
        self.logger_patcher = patch('reasongraph.split.splitter.get_logger', return_value=self.mock_logger)
        self.logger_patcher_sc = patch('reasongraph.split.semantic_chunker.get_logger', return_value=self.mock_logger)
        self.tracker_patcher = patch('reasongraph.split.semantic_chunker.get_progress_tracker', return_value=MagicMock())
    ...
    def test_semantic_chunker_initialization(self, mock_spacy):
        ...
        with patch('reasongraph.split.semantic_chunker.SPACY_AVAILABLE', True):
