...
from reasongraph.ontology.class_inferrer import ClassInferrer
from reasongraph.ontology.property_generator import PropertyGenerator
from reasongraph.ontology.naming_conventions import NamingConventions
from reasongraph.ontology.ontology_generator import OntologyGenerator
from reasongraph.ontology.namespace_manager import NamespaceManager
from reasongraph.ontology.module_manager import ModuleManager
...
class TestOntologyComprehensive(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patchers = [
            patch('reasongraph.ontology.class_inferrer.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.class_inferrer.get_progress_tracker', return_value=self.mock_tracker),
            patch('reasongraph.ontology.property_generator.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.property_generator.get_progress_tracker', return_value=self.mock_tracker),
            patch('reasongraph.ontology.naming_conventions.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.naming_conventions.get_progress_tracker', return_value=self.mock_tracker),
            patch('reasongraph.ontology.ontology_generator.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.ontology_generator.get_progress_tracker', return_value=self.mock_tracker),
        ]
    ...
    def test_owl_generator(self):
        try:
            from reasongraph.ontology.owl_generator import OWLGenerator
        except ImportError:
    ...
    def test_owl_generator_user_facing_schema_compatibility(self):
        ...
        self.assertNotIn("https://reasongraph.dev/ontology/", owl_output)
    ...
    def test_llm_ontology_generator(self):
        try:
            from reasongraph.ontology.llm_generator import LLMOntologyGenerator
        except ImportError:
            self.skipTest("LLMOntologyGenerator not importable")
            
        # Mock provider
        with patch('reasongraph.ontology.llm_generator.create_provider') as mock_create:
    ...
    def test_ontology_engine(self):
        try:
            from reasongraph.ontology.engine import OntologyEngine
        except ImportError:
...
class TestSHACLGeneration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patchers = [
            patch(
                "reasongraph.ontology.ontology_generator.get_logger",
                return_value=self.mock_logger,
            ),
            patch(
                "reasongraph.ontology.ontology_generator.get_progress_tracker",
                return_value=self.mock_tracker,
            ),
        ]
        for p in self.patchers:
            p.start()
        from reasongraph.ontology.ontology_generator import SHACLGenerator
        self.gen = SHACLGenerator(
            base_uri="https://reasongraph.dev/shapes/",
    ...
    def test_generate_returns_shacl_graph(self):
        from reasongraph.ontology.ontology_generator import SHACLGraph
...
class TestSKOSOntologyEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patchers = [
            patch('reasongraph.ontology.engine.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.engine.get_progress_tracker', return_value=self.mock_tracker),
            patch('reasongraph.ontology.ontology_generator.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.ontology_generator.get_progress_tracker', return_value=self.mock_tracker),
            patch('reasongraph.ontology.class_inferrer.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.class_inferrer.get_progress_tracker', return_value=self.mock_tracker),
            patch('reasongraph.ontology.property_generator.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.property_generator.get_progress_tracker', return_value=self.mock_tracker),
            patch('reasongraph.ontology.owl_generator.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.owl_generator.get_progress_tracker', return_value=self.mock_tracker),
            patch('reasongraph.ontology.ontology_evaluator.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.ontology_evaluator.get_progress_tracker', return_value=self.mock_tracker),
            patch('reasongraph.ontology.ontology_validator.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.llm_generator.get_logger', return_value=self.mock_logger),
            patch('reasongraph.ontology.llm_generator.get_progress_tracker', return_value=self.mock_tracker),
            patch('reasongraph.change_management.ontology_version_manager.get_logger', return_value=self.mock_logger),
            patch('reasongraph.change_management.ontology_version_manager.get_progress_tracker', return_value=self.mock_tracker),
        ]
    ...
    def test_list_vocabularies_no_store_raises(self):
        from reasongraph.utils.exceptions import ProcessingError
        from reasongraph.ontology.engine import OntologyEngine
