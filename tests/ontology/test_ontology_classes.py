...
class TestOntologyClasses(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.logger_patcher = patch('reasongraph.ontology.ontology_generator.get_logger', return_value=self.mock_logger)
        self.tracker_patcher = patch('reasongraph.ontology.ontology_generator.get_progress_tracker', return_value=self.mock_progress_tracker)
        
        self.logger_patcher.start()
        self.tracker_patcher.start()

        # Patch for other modules as well
        self.logger_patcher_ci = patch('reasongraph.ontology.class_inferrer.get_logger', return_value=self.mock_logger)
        self.tracker_patcher_ci = patch('reasongraph.ontology.class_inferrer.get_progress_tracker', return_value=self.mock_progress_tracker)
        self.logger_patcher_ci.start()
        self.tracker_patcher_ci.start()

        self.logger_patcher_nc = patch('reasongraph.ontology.naming_conventions.get_logger', return_value=self.mock_logger)
        self.tracker_patcher_nc = patch('reasongraph.ontology.naming_conventions.get_progress_tracker', return_value=self.mock_progress_tracker)
        self.logger_patcher_nc.start()
        self.tracker_patcher_nc.start()

        self.logger_patcher_pg = patch('reasongraph.ontology.property_generator.get_logger', return_value=self.mock_logger)
        self.tracker_patcher_pg = patch('reasongraph.ontology.property_generator.get_progress_tracker', return_value=self.mock_progress_tracker)
