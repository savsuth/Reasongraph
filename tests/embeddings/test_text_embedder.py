...
from reasongraph.embeddings.text_embedder import TextEmbedder
...
class TestTextEmbedder(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_st_patcher = patch('reasongraph.embeddings.text_embedder.SentenceTransformer')
        self.mock_st_class = self.mock_st_patcher.start()
        
        # Create a mock for fastembed.TextEmbedding
        self.mock_fe_patcher = patch('reasongraph.embeddings.text_embedder.TextEmbedding')
        self.mock_fe_class = self.mock_fe_patcher.start()
        
        # Patch availability flags
        self.st_avail_patcher = patch('reasongraph.embeddings.text_embedder.SENTENCE_TRANSFORMERS_AVAILABLE', True)
        self.st_avail_patcher.start()
        
        self.fe_avail_patcher = patch('reasongraph.embeddings.text_embedder.FASTEMBED_AVAILABLE', True)
    ...
    def test_fallback_method(self):
        ...
        with patch('reasongraph.embeddings.text_embedder.SENTENCE_TRANSFORMERS_AVAILABLE', False), \
             patch('reasongraph.embeddings.text_embedder.FASTEMBED_AVAILABLE', False):
