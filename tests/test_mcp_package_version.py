...
separate implementation from `reasongraph.mcp_server` and was not covered
by that fix. `reasongraph.__version__` is the authoritative package
version (see reasongraph/mcp_server/__init__.py), so all three surfaces
...
class TestMCPPackageVersion(unittest.TestCase):
    ...
    def test_schema_info_resource_version_matches_package(self):
        resource = _read_schema_info("reasongraph://schema/info")
