...
from reasongraph.provenance import ProvenanceManager, SourceReference, ProvenanceEntry
from reasongraph.provenance.storage import InMemoryStorage, SQLiteStorage
...
class TestProvenanceManager:
    ...
    def test_default_storage_path_pattern(self, tmp_path):
        ...
        from reasongraph.provenance import default_storage_path, InMemoryStorage, SQLiteStorage
    ...
    def test_default_storage_path_concurrency_safety(self, tmp_path):
        ...
        import threading
        import time
        from reasongraph.provenance import default_storage_path, SQLiteStorage
    ...
    def test_isolation_part_2_no_leakage_after_context(self):
        ...
        from reasongraph.provenance import InMemoryStorage
        assert ProvenanceManager._default_storage_path is None
        prov_mgr = ProvenanceManager()
        assert isinstance(prov_mgr.storage, InMemoryStorage)

    def test_orchestrator_config_wiring(self, tmp_path):
        """Test Reasongraph orchestrator configures ProvenanceManager._default_storage_path."""
        from reasongraph.core import Reasongraph
        from reasongraph.provenance import ProvenanceManager, InMemoryStorage, SQLiteStorage
        ...
        _ = Reasongraph(config={"provenance": {"storage_path": test_db}})
...
class TestAgentTyping:
    ...
    def test_track_entity_agent_record_kwarg(self):
        from reasongraph.provenance import AgentRecord
        prov_mgr = ProvenanceManager()
    ...
    def test_track_entity_default_agent_unchanged(self):
        """No agent kwargs supplied should still default to 'reasongraph' (back-compat)."""
        prov_mgr = ProvenanceManager()
        entry = prov_mgr.track_entity("e1", source="doc1")
        assert entry.agent_id == "reasongraph"
...
class TestTypedActivity:
    ...
    def test_track_entity_activity_record_kwarg(self):
        from reasongraph.provenance import ActivityRecord
        prov_mgr = ProvenanceManager()
...
class TestBundleAndBaseUri:
    ...
    def test_default_base_uri_matches_rdf_exporter_namespace(self):
        from reasongraph.provenance.manager import DEFAULT_BASE_URI
        from reasongraph.export.rdf_exporter import NamespaceManager
        assert NamespaceManager().namespaces["reasongraph"] == DEFAULT_BASE_URI
    ...
    def test_owl_exporter_default_ontology_uri_matches_shared_namespace(self):
        ...
        export_prov()."""
        from reasongraph.provenance.manager import DEFAULT_BASE_URI
        from reasongraph.export.owl_exporter import OWLExporter
...
class TestActivityTimingAcrossWrappers:
    ...
    def test_kg_algorithm_tracker_accepts_caller_supplied_activity_timing(self):
        from reasongraph.kg.kg_provenance import AlgorithmTrackerWithProvenance
    ...
    def test_graph_builder_build_operation_has_no_end_time_yet(self):
        ...
        from reasongraph.kg.kg_provenance import GraphBuilderWithProvenance
    ...
    def test_semantic_extract_wrapper_records_activity_timing(self):
        from reasongraph.semantic_extract.semantic_extract_provenance import ProvenanceMixin
