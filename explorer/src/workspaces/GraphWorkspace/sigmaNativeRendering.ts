...
class EntityTokenNodeProgram extends NodeProgram<(typeof ENTITY_TOKEN_UNIFORMS)[number]> {
  ...
  drawLabel = drawReasongraphNodeLabel;

  drawHover = drawReasongraphNodeHover;
  ...
  processVisibleItem(nodeIndex: number, startIndex: number, data: NodeDisplayData & ReasongraphNodeDrawData): void {
...
export const drawReasongraphNodeLabel: NodeLabelDrawingFunction = (context, rawData) => {
  const data = rawData as typeof rawData & ReasongraphNodeDrawData;
  if (!data.label) {
    return;
  }
...
export const drawReasongraphNodeHover: NodeHoverDrawingFunction = (context, rawData) => {
  const data = rawData as typeof rawData & ReasongraphNodeDrawData;
  if (!data.label) {
    return;
  }
...
export const REASONGRAPH_NODE_PROGRAM_CLASSES = {
  ...DEFAULT_NODE_PROGRAM_CLASSES,
  circle: EntityTokenNodeProgram,
};

export const REASONGRAPH_EDGE_PROGRAM_CLASSES = {
