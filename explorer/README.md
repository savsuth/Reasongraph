# Reasongraph Knowledge Explorer

A browser-based graph workbench for the [Reasongraph](https://github.com/savsuth/Reasongraph) platform. Pan and zoom live graphs, scrub the timeline, trace every decision's causal chain, resolve duplicates, and author your ontology visually. Built on React 19 + Sigma.js.
...
# Custom port
reasongraph-explorer --graph my_graph.json --port 8080

# Suppress auto-open
reasongraph-explorer --graph my_graph.json --no-browser

# Equivalent using python -m
python -m reasongraph.explorer --graph my_graph.json
```

> **Security note:** Since v0.6.5 the Explorer API requires an API key on protected routes. Set the `REASONGRAPH_API_KEY` environment variable and send it as the `X-API-Key` header; without a configured key, protected routes fail closed with `503` rather than serving anonymously. To opt into unauthenticated access for local development only, set `REASONGRAPH_ALLOW_ANONYMOUS=true` explicitly. (`/api/health` and `/api/info` are intentionally unauthenticated.)
>
> The default `--host 127.0.0.1` binds to localhost only, so it is not reachable from other machines on your network. If you bind to `0.0.0.0`, all graph data is readable and writable by any host that can reach the port (subject to API-key auth). The CLI prints a warning when binding to a non-loopback host in anonymous mode or when `REASONGRAPH_API_KEY` is unset.
...
```bash
reasongraph-explorer --graph path/to/my_graph.json --no-browser
```
...
This writes the compiled assets to `../reasongraph/static/`. The Python server then serves the full dashboard at `http://127.0.0.1:8000` — no separate Vite process needed.
...
| `REASONGRAPH_API_KEY` | *(unset)* | API key required on protected routes since v0.6.5; send it as the `X-API-Key` header. When unset, protected routes fail closed with `503`. |
| `REASONGRAPH_ALLOW_ANONYMOUS` | `false` | Set to `true` to opt into unauthenticated access (local development only). |
...
# Type-check and build the production bundle into ../reasongraph/static/
npm run build
...
├── index.html
├── vite.config.ts                     # Dev proxy → 127.0.0.1:8000, build → ../reasongraph/static
└── package.json
```
...
- **If you installed via pip:** `pip install --upgrade "reasongraph[explorer]"` — the wheel includes the pre-built bundle.
...
See the [Contributing Guide](../docs/contributing-guide.md) and open issues on the main [Reasongraph repository](https://github.com/savsuth/Reasongraph).
