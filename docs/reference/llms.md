...
**`reasongraph.llms`** provides a **single consistent API** across every major LLM provider:
...
```python
from reasongraph.llms import Groq, OpenAI, LiteLLM, HuggingFaceLLM
```
...
# pip install "reasongraph[llm-litellm]"
...
`LiteLLM` is the recommended way to access any provider not directly exported by `reasongraph.llms`. Use the `provider/model` string format:
