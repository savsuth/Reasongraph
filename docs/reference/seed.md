...
**`reasongraph.seed`** gives your knowledge graph a **reliable, verified starting point**:
...
<Steps>
  <Step title="Register your seed sources">
    ```python
    from reasongraph.seed import SeedDataManager
...
<Tabs>
  <Tab title="CSV">
    ```python
    from reasongraph.seed import SeedDataSource, SeedDataManager
...
```bash
export REASONGRAPH_SEED_DATA_DIR=./data/seed
export REASONGRAPH_SEED_MERGE_STRATEGY=seed_first
```

<Tip>
  **Use YAML configuration for production deployments.** Hard-coding source paths in Python scripts makes environment-switching (dev → staging → prod) fragile. Declare sources in `config.yaml` under the `seed:` key and override paths with `REASONGRAPH_SEED_DATA_DIR`. This way, the same code runs in every environment.
</Tip>
