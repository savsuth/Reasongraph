...
**`reasongraph.change_management`** provides **enterprise-grade versioning and audit trails** for knowledge graphs and ontologies:
...
<Tabs>
  <Tab title="SQLite (production)">
    ```python
    from reasongraph.change_management import SQLiteVersionStorage, TemporalVersionManager
  ...
  <Tab title="In-Memory (tests)">
    ```python
    from reasongraph.change_management import InMemoryVersionStorage, TemporalVersionManager
...
```python
from reasongraph.change_management.change_log import ChangeLogEntry
...
```python
from reasongraph.change_management import verify_checksum
