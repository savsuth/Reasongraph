...
`reasongraph.semantic_extract` extracts structured information from unstructured text: the foundation of every knowledge graph in Reasongraph:
...
pip install reasongraph[models-huggingface]

# LLM-based extraction (highest accuracy)
pip install reasongraph[llm-groq]    # or llm-openai
```
...
<Tabs>
  ...
  <Tab title="HuggingFace: Custom Models">
    ...
    | | |
    | :-- | :-- |
    | **Setup** | `pip install reasongraph[models-huggingface]` |
  ...
  <Tab title="LLM: Best Accuracy">
    ...
    | | |
    | :-- | :-- |
    | **Setup** | `pip install reasongraph[llm-groq]` + API key |
...
from reasongraph.semantic_extract import EventDetector, Event
