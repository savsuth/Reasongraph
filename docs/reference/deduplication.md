...
**`reasongraph.deduplication`** detects and merges duplicate entities across sources to produce a **clean, single-source-of-truth** knowledge graph:
...
```python
from reasongraph.deduplication import detect_duplicates, merge_entities, calculate_similarity
...
```python
from reasongraph.deduplication import method_registry, SimilarityResult
...
from reasongraph.deduplication import calculate_similarity
...
<Tabs>
  ...
  <Tab title="Large-scale Clustering">
    ```python
    from reasongraph.deduplication import ClusterBuilder, EntityMerger
