...
**`reasongraph.split`** breaks documents into chunks that **preserve semantic context**:
...
Reasongraph's chunking methods are designed to avoid these failure modes.
...
`TextSplitter` can be used standalone or composed manually with other Reasongraph modules. The example below shows a sequential pattern: parse a file, split the text, then extract entities from each chunk:
