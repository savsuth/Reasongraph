...
`reasongraph.vector_store` provides a unified API for storing and searching vector embeddings across all major backends:
...
```bash
pip install "reasongraph[pinecone]"
```
...
```bash
pip install "reasongraph[weaviate]"
```
...
```bash
pip install "reasongraph[qdrant]"
```
...
```bash
pip install "reasongraph[pgvector]"
```
...
<Tabs>
  ...
  <Tab title="Multi-source fusion">
    ```python
    from reasongraph.vector_store import HybridSearch, SearchRanker
  ...
  <Tab title="Multi-tenant namespaces">
    ```python
    from reasongraph.vector_store import VectorStore, NamespaceManager
