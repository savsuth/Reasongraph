...
**`reasongraph.explorer`** is a **browser-based dashboard** for exploring knowledge graphs, managing ontologies, and running visual analyses:
...
<Tip>
  `reasongraph.explorer` is a **server process**, not an importable Python library. Use the CLI or `python -m reasongraph.explorer` to launch. The `app.py` module exposes a module-level `app` instance for use with uvicorn or Docker.
</Tip>
...
The `reasongraph-explorer` command accepts exactly four flags:
...
EXPLORER_CORS_ORIGINS="http://myapp.example.com" \
  reasongraph-explorer --graph my_graph.json --host 0.0.0.0 --port 8080 --no-browser
```
...
- **CLI Launcher** — `reasongraph-explorer --graph my_graph.json` for instant local startup.
...
Install the explorer extras: `pip install "reasongraph[explorer]"`.
...
Path finding requires the `reasongraph[kg]` extras. Install with `pip install "reasongraph[all]"`.
