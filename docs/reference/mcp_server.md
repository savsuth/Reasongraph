---
title: "MCP Server"
description: "Model Context Protocol server: expose Reasongraph's full capability set to Claude Desktop, VS Code, Cursor, and any MCP-aware tool."
icon: "plug"
---

**`reasongraph.mcp_server`** exposes Reasongraph's knowledge graph, decision intelligence, semantic extraction, and reasoning capabilities as an [MCP (Model Context Protocol)](https://modelcontextprotocol.io) **server over stdio**:
...
<Tip>
  `reasongraph.mcp_server` is a **stdio server process**, not a Python library. It exposes no importable classes: all interaction happens through MCP tool calls from a connected AI client.
</Tip>

<Warning>
  **The server communicates over stdio: don't add logging to stdout.** Any `print()` or logger output directed to stdout will corrupt the JSON-RPC message stream. All logging is written to `stderr` only. Configure log verbosity with the `REASONGRAPH_LOG_LEVEL` environment variable.
</Warning>
...
- **3 Readable Resources** — Live graph JSON (`reasongraph://graph/summary`), decision list, and schema/version info: readable by any MCP client.
- **Zero Infrastructure** — Runs over stdio: no server, no port, no Docker required. One config block to activate in any MCP client.
- **Persistent Graphs** — Point `REASONGRAPH_KG_PATH` at a saved graph file to reload it automatically on every server startup.
...
<Steps>
  ...
  <Step title="Add the Reasongraph MCP server config">
    ...
    ```json Cursor
    {
      "mcpServers": {
        "reasongraph": {
          "command": "reasongraph-mcp",
          "env": {
            "REASONGRAPH_KG_PATH": "/path/to/my_graph.json"
          }
        }
      }
    }
    ...
    <Warning>
      **Configure your MCP client's `command` field exactly.** The `command` field must point to the exact executable path (use `which reasongraph-mcp` on macOS/Linux to find it). A wrong path fails silently: the server just doesn't appear in the tools list. Test with the raw `echo | reasongraph-mcp` command first to confirm the binary works.
    </Warning>
...
| :-------- | :------- | :----------- |
| `REASONGRAPH_KG_PATH` | *(none: in-memory graph)* | Path to a persisted graph file to load on startup |
| `REASONGRAPH_LOG_LEVEL` | `WARNING` | Log verbosity: `DEBUG`, `INFO`, `WARNING` |

<Warning>
  **The graph starts empty unless you set `REASONGRAPH_KG_PATH`.** The MCP server creates a fresh in-memory `ContextGraph` on first use. Set `REASONGRAPH_KG_PATH` to a previously saved graph file to restore state across server restarts. Without it, all data is lost when the process exits.
</Warning>

<Tip>
  **Enable debug logging for troubleshooting.** Set `REASONGRAPH_LOG_LEVEL=DEBUG` in your MCP client's `env` block, or run `python -m reasongraph.mcp_server` directly and inspect stderr output.
</Tip>
...
Extract named entities (people, places, organisations, concepts) from text using Reasongraph NER.
...
| URI | Description |
| :--- | :----------- |
| `reasongraph://graph/summary` | High-level graph statistics |
| `reasongraph://decisions/list` | All recorded decisions (up to 50) |
| `reasongraph://schema/info` | Server version and available tools |
...
- [Agno Integration](../integrations/agno) — Use Reasongraph inside Agno multi-agent teams.
