...
`reasongraph.triplet_store` provides **W3C-standard RDF storage** with **SPARQL 1.1** query support. Use it when you need **semantic web compatibility**, OWL-style reasoning, SPARQL-based queries, or standards-compliant RDF serialization.
...
**`TripletStore`** wraps the backend of your choice. Construct a `Triplet` object (from `reasongraph.semantic_extract.types`) and call `add_triplet()`:

```python
from reasongraph.triplet_store import TripletStore
from reasongraph.semantic_extract.types import Triplet
...
<Tabs>
  <Tab title="Oxigraph">
    ```bash
    pip install "reasongraph[tripletstore-oxigraph]"
    ```
  ...
  <Tab title="RDF4J">
    ...
    store = TripletStore(
        ...
        repository_id="reasongraph",   # passed through **config
    )
    ```
...
All store operations use the `Triplet` dataclass from `reasongraph.semantic_extract.types`:
...
<Warning>
  **`add_triplet()` takes a `Triplet` object, not keyword arguments.** Use `Triplet(subject=..., predicate=..., object=...)` from `reasongraph.semantic_extract.types` and pass the object: not `subject=`, `predicate=`, `obj=` to `add_triplet`.
</Warning>
...
from reasongraph.triplet_store import BlazegraphStore
...
```python
from reasongraph.triplet_store import BulkLoader
