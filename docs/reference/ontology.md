...
`reasongraph.ontology` provides the full lifecycle for knowledge graph schemas:
...
- Ontology Hub visual editor available in `reasongraph.explorer` (v0.5.0)
...
```python
from reasongraph.ontology import SHACLGenerator, OntologyValidator, SHACLValidationReport, SHACLViolation
...
<Note>
  Ontology versioning (`VersionManager`, `OntologyVersion`) has moved to `reasongraph.change_management`. Import from there: `from reasongraph.change_management import VersionManager`.
</Note>
