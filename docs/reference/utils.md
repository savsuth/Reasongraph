...
**`reasongraph.utils`** provides **shared infrastructure** used throughout Reasongraph:
...
- Typed exceptions: `ReasongraphError`, `ValidationError`, `ProcessingError`, `ConfigurationError`, `QualityError`
...
| Name | Type | Role |
| :--- | :--- | :--- |
| `setup_logging` | function | Configure the `reasongraph` root logger: accepts `level`, `file`, `console`, `rotation` kwargs |
| `get_logger` | function | Get a named logger instance (`reasongraph.<name>`) |
...
| `ReasongraphError` | exception | Base exception for all Reasongraph errors |
...
- **Exception Hierarchy** — `ReasongraphError` → `ValidationError`, `ProcessingError`: typed exceptions for targeted recovery.
...
<Steps>
  <Step title="Initialize logging at application startup">
    ```python
    from reasongraph.utils import setup_logging, get_logger
    ...
    <Warning>
      **Call `setup_logging(level="INFO")` once at application startup.** Without it, Reasongraph falls back to Python's root logger, which may be silent or misconfigured. Call it before importing other Reasongraph modules to capture initialization messages.
    </Warning>
  </Step>
  <Step title="Instrument expensive functions with the performance decorator">
    ```python
    from reasongraph.utils import log_execution_time
  ...
  <Step title="Configure via environment variables">
    ```bash
    export REASONGRAPH_LOG_LEVEL=DEBUG
    export REASONGRAPH_LOG_FORMAT=json     # "json" | "text"
    export REASONGRAPH_DISABLE_PROGRESS=true
    ```
  </Step>
</Steps>
...
```python
from reasongraph.utils import validate_entity, validate_config, ValidationError
...
```python
from reasongraph.utils import track_progress
...
```python
from reasongraph.utils import clean_text, hash_data, safe_filename
...
```python
from reasongraph.utils import get_nested_value, set_nested_value, merge_dicts
...
```python
from reasongraph.utils import ReasongraphError, ValidationError, ProcessingError
...
except ReasongraphError as e:
    # Catch-all for all Reasongraph framework errors
    logger.error("Framework error: %s", e)
```

| Exception | When Raised | Key Attributes |
| :--------- | :----------- | :-------------- |
| `ReasongraphError` | Base class: all framework errors inherit from this | `.message`, `.context`, `.error_code` |
...
<Tip>
  **Catch `ReasongraphError` as the broadest exception net.** All framework errors inherit from `ReasongraphError`, so `except ReasongraphError` catches validation failures, processing errors, and everything in between. Use specific subclasses for targeted recovery logic.
</Tip>

## File Utilities

```python
from reasongraph.utils import read_json_file
