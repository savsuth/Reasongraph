...
**`reasongraph.conflicts`** detects and resolves **contradictions when multiple sources disagree** on the same fact:
...
Reasongraph's conflict detection makes disagreements explicit and actionable:
...
<Steps>
  ...
  <Step title="Triage by severity">
    ```python
    from reasongraph.conflicts import ConflictAnalyzer
  ...
  <Step title="Auto-resolve low-severity, escalate critical">
    ```python
    from reasongraph.conflicts import ConflictResolver, InvestigationGuideGenerator, ResolutionStrategy
...
```python
from reasongraph.conflicts import ConflictResolver, ResolutionStrategy
...
<Tabs>
  <Tab title="CREDIBILITY_WEIGHTED (recommended)">
    ...
    ```python
    from reasongraph.conflicts import ConflictResolver, SourceTracker, ResolutionStrategy
...
```python
from reasongraph.conflicts import voting, credibility_weighted, most_recent, highest_confidence
...
```python
from reasongraph.conflicts import SourceTracker, SourceReference
...
```python
from reasongraph.conflicts import InvestigationGuideGenerator
...
```python
from reasongraph.conflicts import ConflictType
