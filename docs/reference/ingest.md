...
**`reasongraph.ingest`** is the **universal entry point** for loading data into Reasongraph:
...
<Tabs>
  <Tab title="File-Based">
    ...
    <Warning>
      **`XMLIngestor` is XXE-safe by default.** Do not use standard `xml.etree.ElementTree` to pre-parse XML before passing to Reasongraph: it does not block XXE attacks. `XMLIngestor` uses lxml with `resolve_entities=False` to safely parse untrusted XML.
    </Warning>
  </Tab>
  <Tab title="Web & Feed">
    ### WebIngestor
    ...
    ```python
    from reasongraph.ingest import PublicAPIIngestor, PublicAPIExamples, ingest_public_api
    ...
    ```python
    from reasongraph.ingest import FeedIngestor
  ...
  <Tab title="Cloud Storage">
    ...
    ```python
    from reasongraph.ingest import CloudStorageIngestor
    import os
...
from reasongraph.ingest import ingest_ontology
...
from reasongraph.ingest import FileObject
