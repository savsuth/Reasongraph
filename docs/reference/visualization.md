...
**`reasongraph.visualization`** renders knowledge graphs, ontologies, embedding spaces, and temporal data as **interactive HTML or static images**: without launching the full Explorer server:
...
<Tabs>
  <Tab title="KGVisualizer">
    ...
    <Tip>
      **For interactive dashboards, prefer Explorer.** `KGVisualizer.visualize_network()` generates a self-contained HTML file. The Explorer CLI (`reasongraph-explorer`) gives a full live web app with search, filtering, path-finding, and REST API.
    </Tip>
  ...
  <Tab title="OntologyVisualizer">
    ...
    ```python
    from reasongraph.visualization import OntologyVisualizer
  ...
  <Tab title="EmbeddingVisualizer">
    ...
    ```python
    from reasongraph.visualization import EmbeddingVisualizer
