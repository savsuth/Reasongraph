...
**`reasongraph.core`** is the **coordination layer** for the framework:

- `Reasongraph` orchestrator coordinates the full KG construction pipeline from a YAML config
...
<Tip>
  Use individual modules directly for the vast majority of use cases. Reach for `reasongraph.core` only when you need application-level lifecycle management, centralized config, or a plugin system.
</Tip>


## What You Get

- **Reasongraph** — High-level orchestrator: coordinates the full KG construction pipeline from a single `config.yaml`. Entry point for application-level deployments.
- **ConfigManager** — YAML config with deep-merge, `REASONGRAPH_` env var overrides, and dot-notation nested key access. Keeps secrets out of source files.
...
| Class | Role |
| :--- | :--- |
| `Reasongraph` | Orchestration entry point: coordinates the full KG construction pipeline |
...
## Reasongraph (Orchestration)

**`Reasongraph`** is the high-level entry point that coordinates the **full KG construction pipeline**:

```python
from reasongraph.core import Reasongraph, ConfigManager

config_manager = ConfigManager()
config = config_manager.load_from_file("config.yaml")

framework = Reasongraph(config=config)
...
```python
from reasongraph.core import ConfigManager
...
Environment variable overrides (prefix `REASONGRAPH_`):

Use double underscores (`__`) to produce a dot separator for nested key access. The implementation strips the `REASONGRAPH_` prefix, lowercases the result, and replaces `__` with `.` before calling `set_nested_value()` on the config dict.

```bash
# Double underscores map to nested keys:
export REASONGRAPH_LLM_PROVIDER__MODEL=gpt-4o
export REASONGRAPH_LLM_PROVIDER__NAME=openai
export REASONGRAPH_PROCESSING__BATCH_SIZE=64
export REASONGRAPH_QUALITY__MIN_CONFIDENCE=0.8
export REASONGRAPH_LOGGING__LEVEL=DEBUG
```
...
```python
from reasongraph.core import LifecycleManager
...
```python
from reasongraph.core import method_registry
from reasongraph.core.methods import build_knowledge_base
...
| :-------- | :-------------------- |
| Single extraction task | `from reasongraph.semantic_extract import NERExtractor` |
| Build a knowledge graph | `from reasongraph.kg import GraphBuilder` |
| Multi-step pipeline | `from reasongraph.pipeline import Pipeline` |
| App-level lifecycle + config | `from reasongraph.core import Reasongraph, ConfigManager` |
| Custom dispatch / plugins | `from reasongraph.core import method_registry, PluginRegistry` |

<Tip>
  Use `Reasongraph` and `LifecycleManager` only when building a long-running application (e.g. a FastAPI service) that needs ordered startup, health checks, and graceful shutdown. For scripts and notebooks, use individual modules directly.
</Tip>
