...
`reasongraph.provenance` tracks the full lineage of every fact: from raw ingestion through extraction, chunking, and relationship building:
...
<Tabs>
  <Tab title="In-Memory (default)">
    ...
    ```python
    from reasongraph.provenance import ProvenanceManager, compute_checksum, verify_checksum
...
```python
from reasongraph.provenance import ProvenanceManager, SourceReference
...
```python
from reasongraph.provenance import ProvenanceEntry
...
entry = ProvenanceEntry(
    ...
    agent_id="reasongraph",             # str: default "reasongraph"
...
```python
from reasongraph.provenance import InMemoryStorage, ProvenanceManager
...
```python
from reasongraph.provenance import SQLiteStorage, ProvenanceManager
...
`BridgeAxiom` and `TranslationChain` are available in `reasongraph.provenance.bridge_axiom` for tracking multi-layer domain translations with full coefficient attribution:

```python
from reasongraph.provenance.bridge_axiom import BridgeAxiom, create_translation_chain
...
`GraphBuilderWithProvenance` (from `reasongraph.kg`) automatically records provenance for every node and edge:

```python
from reasongraph.kg import GraphBuilderWithProvenance
from reasongraph.provenance import ProvenanceManager, SQLiteStorage
...
Provenance tracking in Reasongraph produces the following audit artifacts:
