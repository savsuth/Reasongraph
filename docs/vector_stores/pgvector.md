...
**`PgVectorStore`** adds PostgreSQL-native vector storage and similarity search to Reasongraph: no dedicated vector database required.
...
pip install reasongraph[vectorstore-pgvector]
...
```python
# Local development
"postgresql://postgres:postgres@localhost:5432/reasongraph"

# With SSL
"postgresql://user:pass@host/db?sslmode=require"

# Connection parameters
"postgresql://user:pass@localhost/db?connect_timeout=10&application_name=reasongraph"
```
...
```python
from reasongraph.vector_store import PgVectorStore
import numpy as np

# Initialize store
store = PgVectorStore(
    connection_string="postgresql://postgres:postgres@localhost:5432/reasongraph",
...
```bash
# Backup
pg_dump -h localhost -U postgres -d reasongraph -t document_vectors > vectors_backup.sql

# Restore
psql -h localhost -U postgres -d reasongraph < vectors_backup.sql
```
...
export REASONGRAPH_PGVECTOR_URL="postgresql://user:pass@host/db"
```
...
store = PgVectorStore(
    connection_string=os.getenv("REASONGRAPH_PGVECTOR_URL"),
    table_name="vectors",
    dimension=768,
    distance_metric="cosine"
)
