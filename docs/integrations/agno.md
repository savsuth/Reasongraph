...
description: "Wire Reasongraph's semantic intelligence stack into Agno multi-agent teams via five focused components."
icon: "robot"
---

> Five drop-in components that bring Reasongraph's KG, vector memory, and decision intelligence into any Agno agent or team.
...
pip install "reasongraph[agno,graph-neo4j]"
pip install "reasongraph[agno,graph-falkordb]"

# Full stack
pip install "reasongraph[agno,graph-neo4j,vectorstore-pgvector]"
```
...
- **AgnoKnowledgeGraph** — `Agent(knowledge=…)`: Documents flow through the full Reasongraph extraction pipeline into a queryable `ContextGraph` with multi-hop GraphRAG.
...
<Tabs>
  ...
  <Tab title="AgnoKnowledgeGraph">
    Gives Agno agents a queryable `ContextGraph` instead of a flat document store. Ingested documents pass through the full Reasongraph extraction pipeline.
  ...
  <Tab title="AgnoDecisionKit">
    Exposes Reasongraph's decision intelligence as native Agno tools.
...
All five classes are usable without `agno` installed: they carry the full Reasongraph API and degrade gracefully.
