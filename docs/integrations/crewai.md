...
> Three drop-in components that bring Reasongraph's knowledge graph and decision intelligence into any CrewAI crew.
...
Requires `crewai >= 0.80.0`. If `crewai` is not installed, the integration still imports — every class carries the full Reasongraph API and degrades gracefully, but cannot be passed to a `Crew`.

## Components at a Glance

- **ReasongraphKGTool** — `Agent(tools=[…])`: 5 KG construction/query actions: extract entities, extract relations, add to graph, query graph, find related.
- **ReasongraphDecisionTool** — `Agent(tools=[…])`: 5 decision intelligence actions: record decisions, find precedents, trace causal chains, analyze impact, check policies.
- **ReasongraphKnowledgeSource** — `Crew(knowledge_sources=[…])`: Serializes a `ContextGraph` into CrewAI knowledge storage so every agent gets retrieval access to the graph.

## Component Details

<Tabs>
  <Tab title="ReasongraphKGTool">
    ...
    from integrations.crewai import ReasongraphKGTool
    ...
    analyst = Agent(
        ...
        tools=[ReasongraphKGTool(graph=graph)],
    )
  ...
  <Tab title="ReasongraphDecisionTool">
    Exposes Reasongraph's decision intelligence as a native CrewAI tool, backed by `AgentContext`.

    ```python
    from crewai import Agent, Crew, Task
    from integrations.crewai import ReasongraphDecisionTool
  ...
  <Tab title="ReasongraphKnowledgeSource">
    Gives **every agent in the crew** retrieval access to a `ContextGraph`.

    ```python
    from crewai import Agent, Crew, Task
    from reasongraph.context import ContextGraph
    from integrations.crewai import ReasongraphKnowledgeSource
    ...
    crew = Crew(
        agents=[researcher],
        tasks=[...],
        knowledge_sources=[ReasongraphKnowledgeSource(graph=graph)],
    )
    ```
    ...
    **Compatibility:** CrewAI's `BaseKnowledgeSource` contract changed between `0.80.x` and current releases (`load_content()` → `validate_content()`/`aadd()`). `ReasongraphKnowledgeSource` implements both legacy and current methods, so it works across `crewai>=0.80.0`.
  </Tab>
</Tabs>

## Checkpoints & Serialization

CrewAI serializes tools and knowledge sources to JSON for checkpointing/resume. Live Reasongraph state (`ContextGraph`, `AgentContext`, extractors) is **excluded from that serialization** — a restored tool/source comes back with a fresh in-memory `ContextGraph` and logs a warning. Until you re-attach the live graph/context, the restored objects answer queries against an **empty** graph, so re-wire them after resuming (e.g. `restored_tool.graph = live_graph`) before agents continue.

## API Reference

```python
from integrations.crewai import (
    ReasongraphKGTool,          # BaseTool: KG construction/query actions
    ReasongraphDecisionTool,    # BaseTool: decision intelligence actions
    ReasongraphKnowledgeSource, # BaseKnowledgeSource: graph → crew knowledge
    CREWAI_AVAILABLE,         # bool: True if crewai is installed
)
```

All three classes are usable without `crewai` installed: they carry the full Reasongraph API and degrade gracefully.

## See Also

- [Context Module](../reference/context) — AgentContext and ContextGraph backing the integration.
- [Semantic Extraction](../reference/semantic_extract) — NERExtractor / RelationExtractor used by ReasongraphKGTool.
- [LLMs](../reference/llms) — Configure LLM providers for your crew's agents.
- [Vector Store](../reference/vector_store) — Vector backend used by ReasongraphDecisionTool.
