...
**`reasongraph-explorer`** is an **interactive browser dashboard** for knowledge graph exploration. You give it a graph file, it starts a local server, and opens a browser tab where you can search nodes, find paths, inspect provenance, and run analytics: no code required after launch.
...
<Note>
  `pip install reasongraph` alone is not sufficient. Running `reasongraph-explorer` without the `[explorer]` extra immediately prints an error and exits with code 1.
</Note>
...
╭─ Reasongraph Explorer · http://127.0.0.1:8000 ─╮
...
`reasongraph-explorer` accepts exactly four flags:
...
# Short flags
reasongraph-explorer -g my_graph.json -p 8080

# Expose on the network so other machines can connect
reasongraph-explorer --graph my_graph.json --host 0.0.0.0 --port 8080
...
<Warning>
  `--host 0.0.0.0` makes Explorer reachable on every network interface. Since v0.6.5 the Explorer API requires `REASONGRAPH_API_KEY` (sent as the `X-API-Key` header) and fails closed with `503` when unconfigured; unauthenticated access is only possible when `REASONGRAPH_ALLOW_ANONYMOUS=true` is set explicitly. Only use this on a trusted private network.
</Warning>
...
If `reasongraph-explorer` is not on `PATH`, use the module form:

```bash
python -m reasongraph.explorer --graph my_graph.json --port 8080
```
...
reasongraph-explorer --graph /absolute/path/to/my_graph.json
```
...
```bash
reasongraph-explorer --graph my_graph.json --host 0.0.0.0
```
...
- [CLI Setup](cli-setup) — All five Reasongraph executables and when to use each one.
