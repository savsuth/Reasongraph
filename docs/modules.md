---
title: "Modules"
description: "Every Reasongraph module works independently: use only what you need."
icon: "puzzle-piece"
---
...
Reasongraph is organized into **27 modules** across six logical layers. Each module is independently importable: you never pay for what you don't use.
...
```python
from reasongraph.ingest import FileIngestor, WebIngestor, ParquetIngestor, XMLIngestor, DatabricksIngestor
...
<Note>
  `DuckDBIngestor`, `ElasticIngestor`, `GDriveIngestor`, `HuggingFaceIngestor`, `MongoIngestor`, and `PandasIngestor` also ship but aren't re-exported from the top-level `reasongraph.ingest` namespace yet — import them directly, e.g. `from reasongraph.ingest.duckdb_ingestor import DuckDBIngestor`.
</Note>
...
```python
from reasongraph.parse import DocumentParser, DoclingParser
...
```python
from reasongraph.normalize import TextNormalizer, normalize_text, normalize_date
...
```python
from reasongraph.kg import GraphBuilder, GraphAnalyzer, TemporalGraphQuery, SimilarityCalculator
...
```python
from reasongraph.ontology import OntologyGenerator, SHACLGenerator
...
```python
from reasongraph.reasoning import Reasoner, DatalogReasoner
...
```python
from reasongraph.embeddings import EmbeddingGenerator
...
```python
from reasongraph.graph_store import GraphStore
...
```python
from reasongraph.triplet_store import TripletStore
...
```python
from reasongraph.deduplication import EntityResolver
...
```python
from reasongraph.conflicts import ConflictDetector
...
```python
from reasongraph.change_management import TemporalVersionManager
...
```python
from reasongraph.export import RDFExporter, ParquetExporter, ArangoAQLExporter
...
```python
from reasongraph.visualization import KGVisualizer
...
```python
from reasongraph.pipeline import Pipeline
...
```python
from reasongraph.llms import Groq, OpenAI, LiteLLM
import os
...
Exposes Reasongraph as an MCP stdio server for IDE and agent integrations.
...
```python
from reasongraph.seed import SeedManager
...
```python
from reasongraph.evals import KGEvaluator, ExtractionEvaluator, PipelineEvaluator, RegressionTracker
...
```python
from reasongraph.core import Reasongraph, PluginRegistry, ConfigManager

# Top-level orchestrator
sem = Reasongraph(config_path="config.yaml")
sem.initialize()
...
**Components:** `Reasongraph`, `PluginRegistry`, `ConfigManager`, `LifecycleManager`, `HealthMonitor`, `Config`
...
```python
from reasongraph.utils import helpers, validators, logging
```
...
from reasongraph.parse import DocumentParser
from reasongraph.semantic_extract import NERExtractor, RelationExtractor
...
```python
from reasongraph.ingest import FileIngestor
from reasongraph.semantic_extract import NERExtractor
from reasongraph.kg import GraphBuilder
from reasongraph.provenance import ProvenanceManager
...
```python
from reasongraph.ingest import WebIngestor
from reasongraph.normalize import TextNormalizer
from reasongraph.semantic_extract import NERExtractor, RelationExtractor
from reasongraph.graph_store import Neo4jStore
...
```python
from reasongraph.kg import GraphBuilder, TemporalGraphQuery, TemporalVersionManager
...
| [explorer](reference/explorer) | Knowledge Explorer UI | `reasongraph-explorer --graph <file>` |
| [llms](reference/llms) | LLM providers | `Groq`, `OpenAI`, `create_provider` |
| [mcp_server](reference/mcp_server) | MCP stdio server | `python -m reasongraph.mcp_server` |
...
| [core](reference/core) | Base classes & registry | `Reasongraph`, `ConfigManager`, `PluginRegistry`, `LifecycleManager` |
