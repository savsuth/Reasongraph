...
Apache AGE is a PostgreSQL extension that adds graph database functionality, enabling you to run openCypher queries alongside traditional SQL. This backend lets Reasongraph use AGE as a property graph store with the same interface as Neo4j and FalkorDB.
...
<Tabs>
  ...
  <Tab title="Direct (ApacheAgeStore)">
    ...
    node = store.create_node(["Entity"], {"reasongraph_id": "ent-001", "value": "test"})
    print(node)
    # {"id": 844424930131969, "labels": ["Entity"],
    #  "properties": {"reasongraph_id": "ent-001", "value": "test"}}
...
| `GRAPH_STORE_AGE_GRAPH_NAME` | AGE graph name | `reasongraph` |
...
| **Semantic ID** | Application-level identifier. Store it in the `reasongraph_id` property. |

```python
node = store.create_node(
    labels=["Document"],
    properties={"reasongraph_id": "doc-abc-123", "title": "My Doc"},
)
# node["id"] → AGE internal ID (e.g., 844424930131969)
# node["properties"]["reasongraph_id"] → "doc-abc-123"
```

<Warning>Never mix AGE internal IDs with semantic IDs. Use `node["id"]` for graph operations (delete, update, traverse) and `node["properties"]["reasongraph_id"]` for application-level lookups.</Warning>


## Label Handling

AGE supports exactly **one label per vertex**. Reasongraph handles this transparently:
...
All methods match the standard Reasongraph graph store backend interface:
