...
**Reasongraph closes that gap.** It's the context and accountability layer that sits beneath your existing agent framework: not a replacement for LangChain or LlamaIndex, but the infrastructure that makes their outputs trustworthy.
...
## What Reasongraph Adds to Your Stack

Reasongraph gives every agent the infrastructure it needs to be accountable. Drop it into your existing setup in minutes:
...
<img src="/assets/img/diagrams/architecture-overview.svg" alt="Reasongraph four-layer architecture: Ingestion → Processing → Intelligence → Application" style={{ width: '100%', borderRadius: '12px', margin: '24px 0' }} />
...
```python OpenAI
from reasongraph.context import AgentContext, ContextGraph
from reasongraph.vector_store import VectorStore
from reasongraph.llms import OpenAI
...
from reasongraph.context import AgentContext, ContextGraph
from reasongraph.vector_store import VectorStore
from reasongraph.llms import LiteLLM
import os
...
Reasongraph was designed for domains where every decision must be explainable and every fact must be traceable.

<Warning>
  **This is system-level explainability, not foundation-model explainability.** Reasongraph does not expose, reconstruct, or explain what happens *inside* the LLM/foundation model — its internal reasoning or chain-of-thought stays opaque, as it does for any external system. What Reasongraph explains is *outside* the model: the context and data fed in, the decision produced, its provenance, the relevant relationships, the policies applied, and the full execution trail. See [Core Concepts](concepts) for the full scope note.
</Warning>
...
<Steps>
  ...
  <Step title="Learn the mental model">
    ...
    - What GraphRAG is and how Reasongraph implements it
...
- [Installation](installation) — Get Reasongraph installed in under a minute
...
- [Changelog](https://github.com/savsuth/Reasongraph/releases) — Release history
...
| :-------- | :----------------- |
| `reasongraph.context` | Context graphs, agent memory, decision tracking, causal analysis, precedent search |
| `reasongraph.kg` | KG construction, graph algorithms, temporal model, Allen interval algebra |
| `reasongraph.semantic_extract` | NER, relation extraction, event extraction, triplet generation |
| `reasongraph.reasoning` | Forward chaining, Rete, deductive, abductive, SPARQL, Datalog |
| `reasongraph.ontology` | SHACL, SKOS, alignments, diff/migration, auto-generation, OWL/RDF |
| `reasongraph.explorer` | FastAPI Knowledge Explorer, Ontology Hub, Distance Intelligence, SHACL Studio |
| `reasongraph.mcp_server` | MCP stdio server: 12 tools for Claude Desktop, VS Code, Cursor, Windsurf, Cline |
| `reasongraph.vector_store` | FAISS, Pinecone, Weaviate, Qdrant, Milvus, PgVector |
| `reasongraph.graph_store` | Neo4j, FalkorDB, Apache AGE, Amazon Neptune |
| `reasongraph.triplet_store` | In-memory and persistent RDF triple store with SPARQL |
| `reasongraph.ingest` | Files, web, feeds, databases, Snowflake, Parquet, XML, MCP |
| `reasongraph.parse` | Document parsing: PDF, DOCX, HTML, PPTX, Docling layout analysis |
| `reasongraph.split` | Text chunking: sentence, paragraph, token, semantic boundary strategies |
| `reasongraph.normalize` | Text normalization, entity canonicalization, whitespace and encoding cleanup |
| `reasongraph.embeddings` | Sentence-Transformers, FastEmbed, OpenAI, BGE, Ollama local embeddings |
| `reasongraph.pipeline` | Pipeline DSL, parallel workers, retry policies, failure handling |
| `reasongraph.export` | RDF, Parquet, ArangoDB AQL, CSV, OWL, Arrow, GraphML, GEXF, DOT |
| `reasongraph.visualization` | Programmatic graph rendering: force, hierarchical, circular, spring layouts |
| `reasongraph.deduplication` | Entity deduplication v1/v2, similarity scoring, blocking, merging |
| `reasongraph.conflicts` | Conflict detection and resolution across overlapping knowledge sources |
| `reasongraph.provenance` | W3C PROV-O lineage tracking, source attribution, audit trails |
| `reasongraph.change_management` | Version control with SHA-256 checksums, diff, rollback |
| `reasongraph.llms` | Groq, OpenAI, Anthropic, Gemini, Ollama, DeepSeek, Novita AI, LiteLLM, HuggingFace |
| `reasongraph.seed` | Foundation graph seeding from CSV, JSON, SQL, API, and RDF sources |
| `reasongraph.evals` | Evaluation harness: KG quality, extraction F1, pipeline benchmarking, regression tracking |
| `reasongraph.core` | Orchestration, ConfigManager, LifecycleManager, PluginRegistry, MethodRegistry |
| `reasongraph.utils` | Logging, validation, progress tracking, hash utilities, nested dict helpers |
