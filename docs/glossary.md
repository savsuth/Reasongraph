---
title: "Glossary"
description: "Reference definitions for terms and concepts used throughout Reasongraph."
icon: "book"
---
...
A quick-reference dictionary of every concept, data structure, algorithm, and standard referenced in Reasongraph's documentation and codebase.


## Core Concepts

**Agent**
An autonomous AI system that perceives its environment, reasons about information, and takes actions to achieve goals. In Reasongraph, agents use knowledge graphs for structured memory and context, with every decision recorded as a first-class object.

**Context Graph**
A persistent, queryable graph of everything an agent knows, decides, and reasons about: entities, relationships, decisions, and their causal links. The core data structure of `reasongraph.context`.

**Decision**
A first-class object in Reasongraph: a recorded agent choice with category, scenario, reasoning, outcome, confidence score, causal chain, and source provenance. Stored and searchable via `context.record_decision()`.
...
**Chunking**
Splitting large documents into smaller pieces while preserving semantic context. Reasongraph supports recursive, semantic boundary, entity-aware, relation-aware, sliding window, structural, and table-aware chunking strategies.

**Ingestion**
Loading data from external sources: files, databases, APIs, streams: into the pipeline as a unified `SourceDocument`. The first stage in every Reasongraph pipeline.
...
**Abductive Reasoning**
Inference to the most plausible explanation for observed facts. One of six reasoning engines in `reasongraph.reasoning`: returns the most likely hypothesis given available evidence.

**Datalog**
A declarative logic programming language for knowledge base queries. Reasongraph's `DatalogEngine` supports recursive Horn clause rules with bottom-up semi-naive fixpoint semantics. Added in v0.4.0.
...
An AI model trained on large text corpora, capable of understanding and generating natural language. Reasongraph integrates with 8+ LLM providers for entity extraction, relation extraction, and reasoning.
...
Determining when multiple expressions in text refer to the same entity: e.g., "Apple" and "the company" both referring to Apple Inc. Handled by `CoreferenceResolver` in `reasongraph.semantic_extract`.
...
**Event Detection**
Identifying and classifying events in text: acquisitions, partnerships, product launches, regulatory decisions. Handled by `EventDetector` in `reasongraph.semantic_extract`.
...
**Ontology**
A formal specification of domain concepts, relationships, and constraints: typically expressed in OWL. Reasongraph can auto-generate ontologies from knowledge graphs or import existing OWL/RDF/Turtle files.

**Ontology Hub**
Reasongraph's v0.5.0 visual browser UI for the full ontology lifecycle: visual class editor, SHACL Studio, alignment authoring, health dashboard, and version-controlled diffs.

**OWL (Web Ontology Language)**
The W3C standard language for defining and instantiating ontologies. Reasongraph can generate, import, and export OWL ontologies.

**SHACL (Shapes Constraint Language)**
The W3C standard for validating RDF graphs against a set of shape constraints. Reasongraph auto-generates SHACL shapes from ontologies and validates graphs against them.

**SKOS (Simple Knowledge Organization System)**
A W3C standard for representing controlled vocabularies, taxonomies, and thesauri. Used in Reasongraph for domain vocabulary management.
...
**Graph Database**
A database optimized for storing and querying graph-structured data using node and edge primitives. Reasongraph supports Neo4j, FalkorDB, Apache AGE, and Amazon Neptune.
...
**Triplet Store**
A database designed specifically for storing and querying RDF `(subject, predicate, object)` triples. Reasongraph supports embedded Oxigraph as well as Blazegraph, Apache Jena, and RDF4J.

**Vector Store**
A database optimized for storing and searching high-dimensional embedding vectors by similarity. Reasongraph supports FAISS, Pinecone, Weaviate, Qdrant, Milvus, and PgVector.
...
Reasongraph's v0.5.0 feature for semantic neighborhood exploration: N×N distance matrices, ego-mode visualization centered on a single entity, and distance band classification across the graph.
...
**Datalog**
A subset of Prolog used for deductive database queries. Reasongraph's `DatalogEngine` supports recursive Horn clause rules with guaranteed termination via bottom-up semi-naive fixpoint evaluation.

**RDF (Resource Description Framework)**
The W3C standard for representing information as subject-predicate-object triples. The basis of the semantic web and Reasongraph's triplet store.

**SPARQL**
The W3C query language for RDF data. Reasongraph's `SparqlReasoner` uses SPARQL for query-based inference over RDF graphs.


## Data Quality

**Conflict Resolution**
Handling contradictory facts from multiple sources in the same knowledge graph. Reasongraph's `ConflictDetector` surfaces conflicts; resolution strategies include prefer-most-recent, prefer-most-reliable, majority-vote, and flag-for-review.

**Data Provenance**
Complete information about the origin, history, and lineage of every fact: source document, extraction method, timestamp, confidence score. W3C PROV-O compliant in Reasongraph.

**Deduplication**
Identifying and merging duplicate entity records. Reasongraph v2 strategies (`blocking_v2`, `hybrid_v2`, `semantic_v2`) are up to 7x faster than v1.

**W3C PROV-O**
The W3C provenance ontology standard. Reasongraph tracks lineage across all modules in PROV-O compliant format: suitable for HIPAA, SOX, GDPR, and FDA 21 CFR Part 11 compliance.


## Security Terms

**SSRF (Server-Side Request Forgery)**
A vulnerability where a server is induced to make requests to unintended destinations. Reasongraph validates `base_url` at construction time to prevent SSRF in LLM gateway configurations.

**XXE (XML External Entity)**
A vulnerability in XML parsers that allows attackers to read arbitrary files or trigger SSRF. Reasongraph's `XMLIngestor` (v0.5.0) uses an XXE-safe lxml backend.
