...
description: "How to move from the deprecated reasongraph.kg.ProvenanceTracker to the unified reasongraph.provenance.ProvenanceManager."
---

## Why migrate

`reasongraph.kg.ProvenanceTracker` is deprecated and will be removed in a future major version. It was a standalone, in-memory implementation that never delegated to the unified provenance backend — `reasongraph.provenance.ProvenanceManager` is that backend, and is now the supported way to track entity and relationship provenance across every Reasongraph module (see the [Provenance & Audit Trails guide](/guides/provenance)).
