...
description: "How Reasongraph extracts entities, relationships, events, and RDF triplets from unstructured text using pattern, ML, and LLM methods — with fallback chains, batch processing, and real-world pipelines for defense intelligence, cybersecurity, life science, and financial compliance."
...
- `"huggingface"` — domain-specific fine-tuned models at 200 ms–2 s. Use `d4data/biomedical-ner-all` for pharma, `dslim/bert-base-NER` for general high-accuracy NER. Install with `pip install "reasongraph[huggingface]"`.
