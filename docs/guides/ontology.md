...
**Reasoning.** Exporting to OWL/Turtle lets external reasoning engines (HermiT, Pellet, ELK) infer new facts. If Malware subclasses Software and HAMMERTOSS is Malware, an OWL reasoner concludes HAMMERTOSS is also Software. Reasongraph exports the ontology; reasoning itself runs in the external tool.
...
<Info>
Reasongraph's ontology module derives formal OWL ontologies directly from entities and relationships already in your knowledge graph — no schema design upfront. A 6-stage pipeline infers classes, builds hierarchies, maps OWL types, and serialises to Turtle. The pipeline runs in memory; you do not need a running triple store.
</Info>
...
```python
from reasongraph.ontology import validate_ontology
...
```python
from reasongraph.ontology import ClassInferrer, PropertyGenerator
...
```python
from reasongraph.ontology import LLMOntologyGenerator
...
```python
from reasongraph.export import export_owl, export_rdf
...
The exported Turtle file is the input to Reasongraph's SHACL validation pipeline. See the [SHACL Validation](shacl-validation) guide for how to generate constraint shapes from this ontology and run them against live graph data.
...
from reasongraph.context import AgentContext, ContextGraph
from reasongraph.vector_store import VectorStore
from reasongraph.ingest import ingest_file, ingest_web
from reasongraph.ontology import OntologyGenerator, validate_ontology
from reasongraph.export import export_owl, export_rdf
...
```python
from reasongraph.ontology import OntologyGenerator, ClassInferrer
...
```python
from reasongraph.ontology import LLMOntologyGenerator, OntologyGenerator, validate_ontology
from reasongraph.export import export_owl, export_rdf
...
```python
from reasongraph.ontology import LLMOntologyGenerator, validate_ontology
from reasongraph.export import export_owl, export_rdf
