...
description: "Connect Reasongraph to Groq, OpenAI, Anthropic, HuggingFace, Novita AI, and 100+ LLM providers through a unified interface."
---

Reasongraph exposes a unified provider interface — a single `.generate()` method — across Groq, OpenAI, Anthropic Claude, HuggingFace, Novita AI, and 100+ providers via LiteLLM. Use it when you need to swap providers for latency, accuracy, cost, or data-residency reasons without touching application code.

## What Are LLM Integrations?

The `reasongraph.llms` module provides a unified interface for connecting to Large Language Model providers. Instead of learning different APIs for each provider, you use the same methods (`.generate()`, `.generate_structured()`) regardless of whether you're calling Groq, OpenAI, Anthropic, or local HuggingFace models.

**Unified interface across providers:** All LLM providers in Reasongraph expose identical methods, so switching from OpenAI to Anthropic requires changing only the provider constructor, not your application code.

**Provider wrappers vs semantic extraction provider strings:** The `reasongraph.llms` classes (`Groq`, `OpenAI`, `LiteLLM`, `HuggingFaceLLM`) are Python objects for text generation. The `reasongraph.semantic_extract` module accepts provider names as strings for entity and relationship extraction. Both approaches are covered in this guide.
...
<Info>
  The providers in `reasongraph.llms` (`Groq`, `OpenAI`, `LiteLLM`, `HuggingFaceLLM`) are for text generation and `query_with_reasoning()`. For structured entity and relation extraction, `reasongraph.semantic_extract` accepts provider names as strings. Both patterns are covered here.
</Info>
...
This means every place in Reasongraph that accepts an LLM — `query_with_reasoning()`, semantic extraction, custom reasoning loops — accepts any of these providers interchangeably.
...
```python
from reasongraph.llms import Groq
...
```python
from reasongraph.llms import HuggingFaceLLM
...
from reasongraph.llms import Groq, LiteLLM
...
For NER, relation extraction, and triplet extraction, Reasongraph's `reasongraph.semantic_extract` module accepts provider names as strings rather than class instances. The module handles provider instantiation internally.

```python
from reasongraph.semantic_extract import NamedEntityRecognizer, EventDetector
...
Novita AI exposes an OpenAI-compatible API and is available as a built-in provider for the extraction layer. It is accessed differently from the `reasongraph.llms` classes — through `create_provider` from `reasongraph.semantic_extract.providers` — making it the right choice for high-volume NER pipelines where per-call cost matters.

```python
from reasongraph.semantic_extract.providers import create_provider
...
Novita requires the `openai` Python client under the hood — install with `pip install "reasongraph[llm-openai]"` or `pip install openai`.
...
```python
from reasongraph.llms import OpenAI, LiteLLM
from reasongraph.context import AgentContext, ContextGraph
from reasongraph.vector_store import VectorStore
