...
<Info>
  `PipelineBuilder` and `ExecutionEngine` are in `reasongraph.pipeline`. Failure handling, retry policies, and parallelism management are separate classes you can import individually for fine-grained control. Custom step handlers are plain Python functions — no subclassing required.
</Info>
...
```python
from reasongraph.pipeline import ExecutionEngine, FailureHandler, RetryPolicy, RetryStrategy
...
`ExecutionEngine` integrates with Reasongraph's progress tracker automatically — every step start, update, and completion is recorded. To observe progress during a long-running pipeline, inspect step status on the `Pipeline` object after execution:

```python
from reasongraph.pipeline import StepStatus
...
```python
from reasongraph.pipeline import PipelineBuilder, ExecutionEngine, RetryPolicy, RetryStrategy
from reasongraph.ingest import ingest_file
...
from reasongraph.ingest import ingest_file, DBIngestor
from reasongraph.context import AgentContext, ContextGraph
from reasongraph.vector_store import VectorStore
...
from reasongraph.semantic_extract import NamedEntityRecognizer, TripletExtractor
from reasongraph.context import AgentContext, ContextGraph
from reasongraph.vector_store import VectorStore
