...
Export converts Reasongraph graph data into formats used by external tools and systems. Unlike internal persistence mechanisms that keep data within Reasongraph, export is specifically designed for interoperability with external consumers.

**Export vs. internal persistence:**
- **`AgentContext.store()`** and graph persistence keep data inside Reasongraph for continued processing, retrieval, and reasoning
...
**Build once, export many.** Create your knowledge graph through Reasongraph's extraction and reasoning workflows, then export the same graph data to multiple formats for different consumers without rebuilding or reprocessing.

**Interoperability with existing ecosystems.** Connect Reasongraph graphs to established tools and workflows in your organization, from Neo4j graph databases to Gephi visualizations to pandas data analysis pipelines.
...
**Graph database migration and deployment.** Move graphs from Reasongraph's in-memory representation to production graph databases like Neo4j, ArangoDB, or triple stores for scalable query performance.
...
**Use export when:**
- Integrating Reasongraph graphs with external systems and tools
...
- You simply want to save and reload Reasongraph state—use built-in persistence mechanisms instead
...
- Export would add unnecessary complexity to workflows that operate entirely within Reasongraph
...
- Your workflow involves iterative graph building, querying, and reasoning within Reasongraph
- You need to maintain agent memory, conversation history, and decision tracking
- Graph data will continue to be processed and enriched within Reasongraph workflows
...
When your consumers are triple stores (GraphDB, Stardog, Apache Jena) or OWL reasoners (HermiT, Pellet), you want RDF. Reasongraph exports to all five standard RDF serializations through a single `export_rdf` call.

```python
from reasongraph.export import export_rdf
...
GraphML, GEXF, and DOT are the native formats of graph analysis and visualization tools. They preserve node attributes, edge weights, and type labels, so the graph you built in Reasongraph renders immediately in Gephi or NetworkX with full attribute data.

```python
from reasongraph.export import export_graph
...
```python
from reasongraph.export import export_lpg
...
```python
from reasongraph.export import export_arango
...
```python
from reasongraph.export import export_csv
...
```python
from reasongraph.export import export_parquet
...
```python
from reasongraph.export import export_owl
from reasongraph.ontology import OntologyGenerator
...
**Confusing export with persistence.** Export creates external snapshots for interoperability, while persistence maintains Reasongraph's internal state. Don't use export when you need to save and reload agent memory or continue graph-based workflows—use built-in persistence mechanisms instead.
...
from reasongraph.context import AgentContext, ContextGraph
from reasongraph.vector_store import VectorStore
from reasongraph.ingest import ingest_file
from reasongraph.export import export_rdf, export_graph, export_lpg
import os
...
from reasongraph.context import AgentContext, ContextGraph
from reasongraph.vector_store import VectorStore
from reasongraph.export import export_lpg, export_graph
import os
...
from reasongraph.context import AgentContext, ContextGraph
from reasongraph.vector_store import VectorStore
from reasongraph.ingest import DBIngestor
from reasongraph.export import export_rdf, export_csv
...
from reasongraph.vector_store import VectorStore
from reasongraph.ingest import ingest_file
from reasongraph.export import export_arango, export_rdf
import os
