...
embedder   = NodeEmbedder()     # requires: pip install reasongraph[embeddings]
...
These vectors become first-class citizens in Reasongraph — they're stored on `Decision` nodes as `node2vec_embedding` and used automatically by `find_precedents_by_scenario()` as the structural similarity component.
...
```python
from reasongraph.context import ContextGraph
from reasongraph.kg import CentralityCalculator, CommunityDetector, LinkPredictor
from reasongraph.visualization import AnalyticsVisualizer
...
```python
from reasongraph.context import ContextGraph
from reasongraph.kg import CentralityCalculator, LinkPredictor
...
```python
from reasongraph.context import ContextGraph
from reasongraph.kg import CommunityDetector, NodeEmbedder, SimilarityCalculator
