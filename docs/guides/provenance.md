...
description: "How Reasongraph tracks the origin and lineage of every entity, relationship, chunk, and property with W3C PROV-O compliant records — with SHA-256 integrity checksums, SQLite persistence, and cross-module audit trails for defense, pharma, banking, and security compliance."
...
from reasongraph.provenance.storage import SQLiteStorage
...
```python
from reasongraph.provenance.schemas import SourceReference
...
Full version chain (oldest → newest):
  [2024-04-12T14:22:07]  agent=reasongraph
    source=NVD_feed_2024-04-12
    activity=nvd_feed_ingestion
  [2024-04-12T15:18:33]  agent=reasongraph
    source=commercial_feed_2024-04-12
    activity=commercial_feed_ingestion
  [2024-07-18T08:04:11]  agent=reasongraph
    source=NVD_feed_2024-07-18
    activity=nvd_feed_ingestion       # NVD updated their score
  [2024-10-08T09:11:44]  agent=reasongraph
...
```python
from reasongraph.provenance.integrity import compute_checksum
...
`export_prov()` mints entity/agent/activity URIs under `ProvenanceManager.DEFAULT_BASE_URI` (`https://reasongraph.dev/ns#` by default — the same namespace `RDFExporter`'s `NamespaceManager` uses for its `"reasongraph"` prefix, so KG-exported and PROV-exported URIs for the same `entity_id` co-resolve) unless overridden via `export_prov(base_uri=...)` or the CLI's `--base-uri` option.
