---
title: "Core Concepts"
description: "The fundamental ideas behind Reasongraph: knowledge graphs, reasoning, provenance, and temporal intelligence explained."
icon: "book-open"
---
...
Reasongraph transforms unstructured data: documents, web pages, reports, databases: into **knowledge graphs**: structured representations that AI systems can query, reason about, and trace back to sources.

At its core, Reasongraph adds a **context and accountability layer** on top of your existing AI stack. It doesn't replace LangChain, LlamaIndex, or your LLM provider: it makes their outputs **grounded**, **traceable**, and **auditable**.
...
The foundation of everything in Reasongraph. A knowledge graph stores information as three building blocks:
...
<Tabs>
  <Tab title="Knowledge Graph">
    ...
    **Use when:** you need structured reasoning, provenance, compliance, or explainability.
    ...
    ```python
    from reasongraph.kg import GraphBuilder, PathFinder
  ...
  <Tab title="GraphRAG (Both)">
    Reasongraph combines both: vector search seeds the graph traversal, and the graph provides structure and provenance the vector store cannot.
...
Reasongraph uses embeddings for:
...
<Steps>
  ...
  <Step title="Hybrid context retrieval">
    Reasongraph retrieves relevant graph context: entities, typed relationships, and multi-hop reasoning paths: alongside vector-similar text chunks.
  </Step>
...
Reasongraph can auto-generate ontologies from your knowledge graph or import existing OWL/RDF/Turtle ontologies. The **Ontology Hub** (v0.5.0) adds a visual editor, SHACL Studio, alignment authoring, and a live health dashboard. See the [Ontology reference](reference/ontology) for the full 6-stage generation pipeline.


## Reasoning & Inference

Reasongraph includes multiple reasoning engines to derive new knowledge from existing facts.
...
<Tabs>
  <Tab title="Forward Chaining">
    ...
    ```python
    from reasongraph.reasoning import Reasoner, Rule, Fact, RuleType
  ...
  <Tab title="Rete Network">
    ...
    ```python
    from reasongraph.reasoning import ReteEngine
  ...
  <Tab title="Deductive & Abductive">
    ...
    **Abductive**: infers the most likely explanation for observed evidence. Best for diagnostic and investigative use cases.
    ...
    ```python
    from reasongraph.reasoning import GraphReasoner
  ...
  <Tab title="Datalog (v0.4.0)">
    ...
    ```python
    from reasongraph.reasoning import DatalogReasoner, DatalogFact, DatalogRule
...
```python
from reasongraph.kg import TemporalGraphQuery
...
```python
from reasongraph.kg import SimilarityCalculator
...
Real-world data contains the same entity under many names: "Apple", "Apple Inc.", "Apple Computer Inc." Reasongraph's deduplication pipeline detects these, merges attributes, resolves conflicts, and preserves the original source provenance.
...
Every fact in Reasongraph links back to:
...
Every agent decision is a first-class object in Reasongraph: recorded, causally linked, and searchable by precedent. This is the **accountability layer** for AI pipelines: decisions are no longer ephemeral log messages, they are queryable knowledge graph nodes.
...
When multiple sources disagree on the same fact, Reasongraph flags and resolves the conflict rather than silently picking one value.
...
Reasongraph is designed for extension. Any component: ingestor, extractor, graph builder, reasoning engine: can be replaced or augmented with a custom implementation registered at runtime.

<AccordionGroup>
  <Accordion title="PluginRegistry: replace any component by name">

    `PluginRegistry` provides dynamic plugin discovery, registration, and loading across all modules. Register your own class under a string key; Reasongraph will use it wherever that key is referenced in config or pipeline steps.

    ```python
    from reasongraph.core import PluginRegistry
  ...
  <Accordion title="MethodRegistry: add domain-specific graph operations">
    ...
    ```python
    from reasongraph.kg import MethodRegistry
