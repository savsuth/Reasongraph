...
description: "Map your goal to the right Reasongraph module in under 30 seconds."
icon: "compass"
---
...
| Use Reasongraph from Claude Desktop or Cursor | `mcp_server` | `reasongraph-mcp` |
| Bootstrap a graph from verified seed data | `seed` | `SeedDataManager` |
| Extend Reasongraph with a custom component | `core` | `PluginRegistry` |
...
<Tabs>
  ...
  <Tab title="Export">
    ...
    **Module:** `export`
    ...
    ```python
    from reasongraph.export import RDFExporter, ParquetExporter, LPGExporter, ArangoAQLExporter
  ...
  <Tab title="MCP — Claude / Cursor">
    Use Reasongraph from Claude Desktop, Cursor, VS Code, or any MCP-aware tool — no Python code required after setup. 12 tools available instantly.
    ...
    **Available tools:** `extract_entities` · `extract_relations` · `add_entity` · `add_relationship` · `record_decision` · `query_decisions` · `find_precedents` · `get_causal_chain` · `run_reasoning` · `get_graph_analytics` · `export_graph` · `get_graph_summary`
    ...
    <Warning>
      Set `REASONGRAPH_KG_PATH` to persist your graph across restarts. Without it, all data is lost when the server process exits.
    </Warning>
...
<AccordionGroup>
  ...
  <Accordion title="I'm adding Reasongraph to an existing agent — what's the minimum?" icon="plug">
