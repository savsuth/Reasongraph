---
title: "FAQ"
description: "Common questions about Reasongraph: installation, features, integrations, and troubleshooting."
...
| Works with LangChain / LlamaIndex? | Yes: Reasongraph is a layer on top, not a replacement |
...
<Accordion title="What is Reasongraph?" icon="info-circle">

Reasongraph is an open-source framework for building context graphs and decision intelligence layers for AI. It transforms unstructured data: documents, APIs, databases: into structured knowledge graphs with full provenance tracking, making AI systems explainable and auditable.
...
<Accordion title="What can I build with Reasongraph?" icon="hammer">
...
<Accordion title="What makes Reasongraph different from LangChain or LlamaIndex?" icon="scale-balanced">

Most frameworks stop at retrieval or generation. Reasongraph adds an **accountability layer**: every decision is recorded, every fact links to a source, and every reasoning step is explainable. It's designed for environments where you need to audit *why* an AI reached a conclusion: not just what it said.

Reasongraph works alongside these frameworks, not against them.

</Accordion>

<Accordion title="Does Reasongraph explain an LLM's internal reasoning or chain-of-thought?" icon="triangle-exclamation">

No. This is **system-level explainability, not foundation-model explainability**. Reasongraph does not expose, reconstruct, or explain what happens *inside* the LLM/foundation model — its internal reasoning or chain-of-thought stays opaque, as it does for any external system.

What Reasongraph explains is *outside* the model: what context and data were used, what decision was produced, the provenance behind it, the relevant relationships, the policies applied, and the resulting decision trail.

In short: Reasongraph explains and audits *what the AI system did* — not the foundation model's private internal reasoning.

</Accordion>

<Accordion title="Is Reasongraph free?" icon="tag">

Yes: MIT licensed, no vendor lock-in, no paywalled features. Some capabilities require third-party API keys (e.g., OpenAI embeddings, Groq inference), but Reasongraph itself is always free and open source.
...
<Accordion title="How do I install Reasongraph?" icon="download">
...
If you're on an older version, install extras individually: `pip install "reasongraph[core]"`, then add `[llm-openai]`, `[gpu]`, etc.
...
<Accordion title="What data sources does Reasongraph support?" icon="database">
...
<Accordion title="Does Reasongraph support GPUs?" icon="bolt">
...
<Accordion title="How does Reasongraph handle large datasets?" icon="layer-group">
...
```python
from reasongraph.kg import TemporalKnowledgeGraph
...
A visual browser UI for the full ontology lifecycle: launched via `reasongraph.explorer`. Includes:
...
<Accordion title="Is Reasongraph production-ready?" icon="shield-check">
...
<Accordion title="ModuleNotFoundError: No module named 'reasongraph'" icon="xmark-circle">
...
```python
from reasongraph.graph_store import FalkorDBStore
...
- [GitHub Issues](https://github.com/savsuth/Reasongraph/issues) — Bug reports and feature requests.
- [Contributing](contributing-guide) — Help improve Reasongraph.
