"""
Reasongraph MCP Server — JSON-RPC 2.0 over stdio.
...
can interact with the Reasongraph knowledge graph.
...
log = logging.getLogger("reasongraph.mcp.server")
...
class ReasongraphMCPServer:
    """Reasongraph MCP server — reads JSON-RPC requests from stdin, writes to stdout."""
    ...
    def run(self) -> None:
        ...
        log.info("Reasongraph MCP server starting (stdio)")
...
def main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="Reasongraph MCP Server")
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    args = parser.parse_args()
    ReasongraphMCPServer(debug=args.debug).run()
