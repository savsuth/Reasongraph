...
log = logging.getLogger("reasongraph.mcp.tools.extraction")
...
def handle_extract_entities(args: dict) -> dict:
    """Extract named entities from text using Reasongraph NER."""
...
def handle_extract_relations(args: dict) -> dict:
    ...
    _clear_cache()
    try:
        from reasongraph.semantic_extract import NamedEntityRecognizer, RelationExtractor, TripletExtractor
        entities = NamedEntityRecognizer().extract(text) or []
...
EXTRACTION_TOOLS = [
    ...
    {
        "name": "extract_all",
        "description": "Run the full Reasongraph extraction pipeline: NER, coreference resolution, relation extraction, event detection, and triplet generation.",
