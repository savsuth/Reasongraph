...
railway variable --set "REASONGRAPH_API_KEY=$(openssl rand -hex 32)"
railway up
```
...
Railway exposes this service on a public domain, so `REASONGRAPH_API_KEY` is required — without it the Explorer refuses every protected route (503) rather than serving anonymously. Pass the same value as the `X-API-Key` header from any client that talks to the deployed API.
