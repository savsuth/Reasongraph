---
title: "[Q&A] "
labels: ["question", "help wanted"]
---

## Question

Please provide a clear and detailed question. Be specific about what you're trying to accomplish.

## Objective

Describe your end goal or what you're trying to achieve.

## Attempts

List the steps you have already taken to solve this problem:

1. 
2. 
3. 

## Code Example

If your question involves code, please share a minimal, reproducible example:

```python
from reasongraph import Reasongraph

# Your code here
```

## Error Messages

If applicable, paste any error messages or describe unexpected behavior:

```
# Paste error messages here
```

## Environment

- Python version: 
- Reasongraph version: 
- OS: 
- Relevant dependencies: 

## Checklist

- [ ] I have searched existing [discussions](https://github.com/savsuth/Reasongraph/discussions) and [issues](https://github.com/savsuth/Reasongraph/issues)
- [ ] I have checked the [documentation](https://github.com/savsuth/Reasongraph/tree/main/docs) and [FAQ](https://github.com/savsuth/Reasongraph/blob/main/docs/faq.md)
- [ ] I have provided a minimal code example (if applicable)
- [ ] I have included error messages (if applicable)
- [ ] I have provided environment details

